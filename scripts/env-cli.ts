...
// program
  ...
  .action((: any) => {
    ...
    if(status.summary.missing > 0: any): any {
      );}
    if(status.summary.warnings > 0: any): any {
      );}
  });
...
// program
  ...
  .action((options: any) => {
    ...
    for(const entry of recent: any): any {
...
// program
  ...
  type: 'confirm',
    ...
    if (confirm: any) {
      ...
      } catch (error: any) { spinner.fail('Compact failed');
        console.error(error);
        process.exit(1);
       });
...
// program
  ...
  .action(async (: any) => {
    ...
    const missingVars = validation.errors.filter((e: any) => e.severity === 'error' && e.message.includes('missing'));
    ...
    for(const error of missingVars: any): any {
...
// program
  ...
  .action((: any) => {
    ...
    for(const error of validation.errors: any): any {
      if (error.severity === 'error' && criticalServices.includes(error.service)) {
        );
        criticalErrors++;}}
    if(criticalErrors > 0: any): any {
      );
      process.exit(1);
    } else if (validation.errors.filter((e: any) => e.severity === 'error').length > 0) {
      );
    } else {
      );}
  });
