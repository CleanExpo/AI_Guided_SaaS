...
class DailyHealthCheck {
    ...
    if (fs.existsSync(historyFile)) {
      try {
        ...
        if (lastCheck: any) {
          ...
          if(metric && typeof metric.value === 'number': any): any {
            return metric.value;
           } catch (error: any) {
        // Ignore parse errors
}
  ...
  private displaySummary() {
    ...
    if(tsErrors && typeof tsErrors.value === 'number': any): any {
      const _target = 5000; // Week 1 target
      if(tsErrors.value > target: any): any {
