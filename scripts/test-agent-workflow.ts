...
import { AgentLoader, discoverAllAgents, loadExecutionChain } from '../src/lib/agents/AgentLoader'
import { AgentRegistry, initializeAgentRegistry, findBestAgent } from '../src/lib/agents/AgentRegistry'
...
class AgentWorkflowTester {
  ...
  private async testAgentDiscoveryAndLoading(): Promise<void> {
    ...
    await this.runTest(suite, 'Load Agent by Role', async () => {
    ...
    await this.runTest(suite, 'Load Execution Chain', async () => {
      ...
      return {
        ...
        agent_roles: chain.map(a => a.role),
        priorities: chain.map(a => a.priority)
      }
    })

    // Test 1.4: Get required agents for stage
    await this.runTest(suite, 'Get Required Agents for Stage', async () => {
      ...
      return {
        ...
        roles: requiredAgents.map(a => a.role),
        highest_priority: Math.min(...requiredAgents.map(a => a.priority))
      }
    })
  ...
  private async testAgentRegistryManagement(): Promise<void> {
    ...
    await this.runTest(suite, 'Auto-Register Agents', async () => {
      const registeredCount = await initializeAgentRegistry()
    ...
    await this.runTest(suite, 'Find Best Agent for Task', async () => {
    ...
    await this.runTest(suite, 'Query Agents with Criteria', async () => {
      ...
      return {
        ...
        roles: coreAgents.map(a => a.agent.role),
        health_statuses: coreAgents.map(a => a.health_status)
      }
    })

    // Test 2.4: Update agent metrics
    await this.runTest(suite, 'Update Agent Metrics', async () => {
  ...
  private async testAgentCommunication(): Promise<void> {
    ...
    await this.runTest(suite, 'Request-Response Pattern', async () => {
      const responsePromise = this.communication.sendRequest(
        'ARCHITECT',
        'BACKEND',
    ...
    await this.runTest(suite, 'Create Communication Channel', async () => {
      const channelId = this.communication.createChannel(
        'development_team',
    ...
    await this.runTest(suite, 'Agent Handoff Protocol', async () => {
      const handoffSuccess = await performAgentHandoff(
        'ARCHITECT',
        'FRONTEND',
        'architecture',
        {
      ...
      return {
        handoff_successful: handoffSuccess,
  ...
  private async testAgentCoordination(): Promise<void> {
    ...
    await this.runTest(suite, 'Execute Coordination Plan', async () => {
    ...
    await this.runTest(suite, 'Get Coordination Status', async () => {
  ...
  private async testAgentMonitoring(): Promise<void> {
    ...
    await this.runTest(suite, 'Perform Health Checks', async () => {
      ...
      const healthyCount = healthStatuses.filter(hc => hc.status === 'healthy').length

      return {
        agents_checked: healthChecks.size,
        ...
        health_distribution: {
          healthy: healthStatuses.filter(hc => hc.status === 'healthy').length,
          warning: healthStatuses.filter(hc => hc.status === 'warning').length,
          critical: healthStatuses.filter(hc => hc.status === 'critical').length,
          offline: healthStatuses.filter(hc => hc.status === 'offline').length
        }
      }
    })

    // Test 5.3: Create and acknowledge alert
    await this.runTest(suite, 'Create and Acknowledge Alert', async () => {
      ...
      const acknowledged = this.monitor.acknowledgeAlert(alertId, 'test_system')
    ...
    await this.runTest(suite, 'Get Performance Trends', async () => {
  ...
  private async testEndToEndIntegration(): Promise<void> {
    ...
    await this.runTest(suite, 'Full Agent Lifecycle', async () => {
      ...
      const messageId = await this.communication.sendMessage({
        from_agent: 'system',
        to_agent: 'ARCHITECT',
        type: 'notification',
    ...
    await this.runTest(suite, 'Complete Project Simulation', async () => {
      const projectDescription = 'Build a task management application with user authentication, task CRUD operations, and team collaboration features'
    ...
    await this.runTest(suite, 'System Stress Test', async () => {
      const messageCount = 50
      const messagePromises: Promise<string>[] = []

      // Send multiple messages concurrently
      for (let i = 0; i < messageCount; i++) {
        const promise = this.communication.sendMessage({
  ...
  private finalizeTestSuite(suite: TestSuite): void {
    const successCount = suite.tests.filter(t => t.success).length
