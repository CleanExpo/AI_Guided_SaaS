...
class SystematicSyntaxFixer {
  ...
  async fixFile(filePath) {
    try {
      ...
      const _typeInConditionRegex = /\b(if|while|for)\s*\(\s*([^)]+?):\s*(any|unknown|string|number|boolean|object)\s*\)/g;
      ...
      const _catchTypeRegex = /}\s*catch\s*\(\s*([^)]+?):\s*(any|unknown|Error)\s*\)/g;
      ...
      const _missingCommaRegex = /(\w+)\s+(\w+\s*:)/g;
      ...
      const _objectPropertyRegex = /{([^}]*?)(\w+)\s+(\w+)([^}]*?)}/g;
