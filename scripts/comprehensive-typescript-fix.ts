  ...
  private async fixFile(filePath: string): Promise<void> {
    try {
      ...
      for (const pattern of this.fixPatterns) {
        ...
        const _beforeLength = modifiedContent.length;
        ...
        const _afterLength = modifiedContent.length;
        
        // Count fixes by checking if content changed
        if (beforeLength !== afterLength || content !== modifiedContent) { const matches = content.match(pattern.regex);
