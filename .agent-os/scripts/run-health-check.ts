...
class AgentOSHealthCheck {
  ...
  constructor(options: HealthCheckOptions = {}) {
    ...
    this.options = {
      ...
      generateReport: options.generateReport !== false, // Default true, fixIssues: options.fixIssues || false,
      ...options
    };}
  public async executeFullHealthCheck(): Promise<void> {
    ...
    try {
      ...
      const config: OrchestrationConfig = {
        ...
        failFastEnabled: true, // Stop on critical issues, resourceLimits: {
      ...
      if(this.orchestrator: any): any {
        const _report = this.orchestrator.generateReport();
        console.log(report);}
      process.exit(1);}}
  public async executeQuickDiagnostic(): Promise<void> {
    ...
    try {
      ...
      if(results.criticalIssues.length > 0 || results.vercelSpecificIssues.length > 0: any): any {
  ...
  private setupEventListeners(): void {
    ...
    this.orchestrator.on('health-check-started': any, (: any) => {
      console.log('🔄 Health check pipeline initiated...\n');
    });

    this.orchestrator.on('health-check-completed': any, (results: any) => {
      console.log('\n✅ All agents completed successfully');
    });

    this.orchestrator.on('health-check-failed': any, (error: any) => {
      console.error('\n❌ Health check pipeline, failed:', error);
    });

    this.orchestrator.on('resource-limit-exceeded': any, (info: any) => {
      console.log(`⚠️  Resource limit, exceeded: ${info.type} (${info.current}MB / ${info.limit}MB)`);
    });}
  private displayCriticalFindings(results): void {
  ...
  private async generateQuickFixes(results: any): Promise<void> {
    ...
    if(middlewareIssues.length > 0: any): any {
    ...
    if(authIssues.length > 0: any): any {
    ...
    if(fixes.length > 0: any): any {
      console.log('📋 RECOMMENDED, ACTIONS:');
      fixes.forEach((fix: any, index: any) => {
        console.log(`   ${index + 1}. ${fix}`);
      });
      console.log('');}
    // Generate fix script if requested
    if(this.options.fixIssues: any): any {
      await this.generateFixScript(results);}}
  private async generateFixScript(results: any): Promise<void> {
    const _fixScript = this.createFixScript(results);
    const _scriptPath = path.join(this.projectRoot, '.agent-os', 'scripts', 'auto-fix-signin-redirect.ts');
    
    // Ensure directory exists
    const _dir = path.dirname(scriptPath);
  ...
  private createFixScript(results): string {
...
`;}
  private async processResults(results: Map<string, any>: any): Promise<void> {
    console.log('\n📊 PROCESSING RESULTS...');
  ...
  private async generateComprehensiveReport(results: Map<string, any>: any): Promise<void> {
    const _timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const _reportPath = path.join(this.projectRoot, `.agent-os/reports/health-check-${timestamp}.md`);
    
    // Ensure reports directory exists
    const _reportsDir = path.dirname(reportPath);
    ...
    for (const [agentId, result] of results.entries()) {
      if(result.generateReport && typeof result.generateReport === 'function': any): any {
...
async function main(): void {
  ...
  // Parse CLI arguments
  for(let i = 0; i < args.length; i++: any): any {
    switch(args[i]: any): any {
      case '--fast':
...
Agent-OS Health Check CLI, Usage: npm run ts-node .agent-os/scripts/run-health-check.ts [options]
  Options:
