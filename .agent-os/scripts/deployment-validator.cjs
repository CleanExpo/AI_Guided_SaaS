...
class DeploymentValidator {
  ...
  scanForHydrationIssues(dir, issues) {
    ...
    for (const file of files) {
      ...
      if (stat.isDirectory()) {
        ...
        this.checkFileForHydrationIssues(filePath, issues);
      }
    }
  }
  ...
  checkFileForHydrationIssues(filePath, issues) {
    ...
    const hasHooks = /useState|useEffect|useRef|useContext/.test(content);
    ...
    const hasBrowserAPIs = /window\.|document\.|localStorage|sessionStorage/.test(content);
  ...
  scanForImportExportIssues(dir, issues) {
    ...
    for (const file of files) {
      ...
      if (stat.isDirectory()) {
        ...
        this.checkImportExportConsistency(filePath, issues);
      }
    }
  }
  ...
  checkImportExportConsistency(filePath, issues) {
    ...
    const hasNamedExports = /export \{|export const|export function|export class/.test(content);
