...
export interface UseRAGReturn {
  // Query operations,
    query: (question: string, options?: Partial<RAGQuery>) => Promise<RAGResponse>,</RAGResponse>
  streamQuery: (question: string, options?: Partial<RAGQuery>) => AsyncGenerator<string void, unknown></string>
  // Document operations,
    addDocument: (content: string, metadata) => Promise<string>, addFromUrl: (url: string) => Promise<string[], >,</string>
  addFromFile: (file: File) => Promise<string;>,</string>
  ingestCodebase: (path: string, options?) => Promise<{ documentsAdded: number;
  errors: string[] }>
  // Knowledge base management, updateDocument: (id: string, content?: string, metadata?) => Promise<any></any>
deleteDocument: (id: string) => Promise<any></any>
  getSimilar: (documentId: string, topK?: number) => Promise<any[]></any>
getStats: () => Promise<any></any>
  exportKnowledge: (format? null : 'json' | 'markdown') => Promise<string></string>
clearKnowledge: () => Promise<any></any>
...
const _addFromUrl  = useCallback(async (url: string): Promise<string[]> => {</string>
    setLoading(true, setError(null), try {;
      const engine = await initializeEngine();
...
const _addFromFile  = useCallback(async (file: File): Promise<string> => {</string>
    setLoading(true, setError(null), try {;
...
const _deleteDocument = useCallback(async (id: string): Promise<any> => {</any>
    setLoading(true, setError(null), try {;
      const engine = await initializeEngine();
...
const _getSimilar = useCallback(async ();
    ...
    setLoading(true, setError(null), try {
      ...
      const _message = err instanceof Error ? err.message : 'Failed to get similar documents', setError(message); throw err
} finally {
      setLoading(false)}, [initializeEngine])
  // Get knowledge base statistics;

const _getStats = useCallback(async (): Promise<any> => {</any>
    setLoading(true, setError(null), try {;
      const engine = await initializeEngine();
      return await engine.getStats()
}; catch (err) {
      const _message = err instanceof Error ? err.message : 'Failed to get stats', setError(message); throw err
} finally {
      setLoading(false)}, [initializeEngine])
...
const _clearKnowledge = useCallback(async (): Promise<any> => {</any>
    setLoading(true, setError(null), try {;
      const engine = await initializeEngine();
      await engine.clear();
...
getDocumentTypeFromFile(file: File): 'code' | 'documentation' | 'tutorial' | 'api' | 'article' | 'other' {
  const _extension = file.name.split('.').pop()?.toLowerCase(); const codeExtensions = ['js', 'jsx', 'ts', 'tsx', 'py', 'java', 'cpp', 'c', 'go', 'rs', 'rb', 'php'], const docExtensions  = ['md', 'rst', 'txt'];
