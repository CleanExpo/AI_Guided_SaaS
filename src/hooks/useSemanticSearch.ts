...
export function useSemanticSearch(options: UseSemanticSearchOptions = {}): UseSemanticSearchOptions = {}) { const [isSearching, setIsSearching]: any[] = useState<any>(false);
  const [isIndexing, setIsIndexing]: any[] = useState<any>(false);
  const [searchResults, setSearchResults]: any[] = useState<SearchResponse | null>(null);
  const [context7, setContext7]: any[] = useState<string[]>([]);
