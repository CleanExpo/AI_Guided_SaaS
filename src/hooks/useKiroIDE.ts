...
export interface UseKiroIDEReturn {
  ...
  disconnect: () => void
  // Project management,
    createProject: (project) => Promise<KiroProject >,</KiroProject>
  openProject: (projectId: string) => Promise<KiroProject></KiroProject>
  saveProject: (projectId: string) => Promise<any;>,</any>
  listProjects: () => Promise<KiroProject[];></KiroProject>
  // File operations,
    readFile: (path: string) => Promise<KiroFile></KiroFile>
  writeFile: (path: string,;
  content: string) => Promise<any;>,</any>
  createFile: (path: string, content?: string) => Promise<any>,</any>
  deleteFile: (path: string) => Promise<any;>,</any>
  renameFile: (oldPath: string;
  newPath: string) => Promise<any;>,</any>
  getFileTree: (projectId: string) => Promise<KiroFileTree></KiroFileTree>
  // Terminal operations,
    createTerminal: (config?: Partial<KiroTerminal>) => Promise<KiroTerminal></KiroTerminal>
  executeCommand: (terminalId: string,;
  command: string) => Promise<any;>,</any>
  closeTerminal: (terminalId: string) => Promise<any;></any>
  // AI assistance,
...
runDiagnostics: (projectId: string) => Promise<KiroDiagnostic[];>,</KiroDiagnostic>
  ...
  // Debugging,
    startDebugSession: (config) => Promise<KiroDebugSession></KiroDebugSession>
  setBreakpoint: (file: string;
  line: number, condition?: string) => Promise<any>,</any>
  stepOver: (sessionId: string) => Promise<any;>,</any>
  stepInto: (sessionId: string) => Promise<any;>,</any>
  stepOut: (sessionId: string) => Promise<any;>,</any>
  continue: (sessionId: string) => Promise<any;>,</any>
  stopDebugSession: (sessionId: string) => Promise<any;></any>
  // State,
    loading: boolean;
  error: string | nul
l, currentProject: KiroProject | nul
l,
    openFiles: KiroFile[],
...
const [openFiles, setOpenFiles]  = useState<KiroFile[]>([]);</KiroFile>

const [terminals, setTerminals] = useState<KiroTerminal[]>([]);</KiroTerminal>
  
const [debugSessions, setDebugSessions] = useState<KiroDebugSession[]>([]);</KiroDebugSession>
  // Initialize client
  useEffect(() => {
    const kiroClient = getKiroClient(, setClient(kiroClient), // Set up event listeners;
...
const _handleFileChanged  = (data: { path: string, content: string
  ...
  const _handleTerminalCreated = (terminal: KiroTerminal) => { setTerminals(prev => [...prev, terminal])};
  const _handleTerminalClosed = (terminalId: string) =>  {
    setTerminals(prev => prev.filter((t) => t.id !== terminalId))
 };
  const _handleDebugSessionStarted = (session: KiroDebugSession) => { setDebugSessions(prev => [...prev, session])};
...
const _connect = useCallback(async () => {
    if (!client) {r}eturn setLoading(true, setError(null), try {;
...
const _disconnect = useCallback(() => {
    if (!client) {r}eturn client.disconnect(, setConnected(false)}, [client]);
  // Project management;

const _createProject = useCallback(async (project) => {
    if (!client) {t}hrow new Error('Client not initialized', setLoading(true); setError(null);
    try {
...
const _listProjects = useCallback(async () => {
    ...
    try {
      ...
      const _message = err instanceof Error ? err.message : 'Failed to list projects', setError(message); throw err
} finally {
      setLoading(false)}, [client])
  // File operations;

const _readFile = useCallback(async (path: string) => { if (!client) {t}hrow new Error('Client not initialized', try {;
...
const _getFileTree = useCallback(async (projectId: string) =>  {
    if (!client) {t}hrow new Error('Client not initialized', try {;
      return await client.getFileTree(projectId)}; catch (err) {
      const _message = err instanceof Error ? err.message : 'Failed to get file tree', throw new Error(message)}, [client]);
  // Terminal operations;

const _createTerminal = useCallback(async (config? null : Partial<KiroTerminal>) =>  {</KiroTerminal>
    if (!client) {t}hrow new Error('Client not initialized', try {;
      const terminal = await client.createTerminal(config), setTerminals(prev => [...prev, terminal]);
      return terminal
}; catch (err) {
...
const _executeCommand = useCallback(async (terminalId: string, command: string) =>  {
    if (!client) {t}hrow new Error('Client not initialized', try {;
      await client.executeCommand(terminalId, command)}; catch (err) {;
      const _message = err instanceof Error ? err.message : 'Failed to execute command', throw new Error(message)}, [client]);

const _closeTerminal = useCallback(async (terminalId: string) =>  {
    if (!client) {t}hrow new Error('Client not initialized', try {
      await client.closeTerminal(terminalId), setTerminals(prev => prev.filter((t) => t.id !== terminalId))
}; catch (err) {
      const _message = err instanceof Error ? err.message : 'Failed to close terminal', throw new Error(message)}, [client]);
...
const _getAISuggestions = useCallback(async (file: string, position?) => {
    if (!client) {t}hrow new Error('Client not initialized');
        return client.getAISuggestions(file, position)}, [client]);
...
const _getCompletions = useCallback(async (file: string, position) => {
    if (!client) {t}hrow new Error('Client not initialized');
        return client.getCompletions(file, position)}, [client]);

const _runDiagnostics = useCallback(async (projectId: string) => {
    if (!client) {t}hrow new Error('Client not initialized');
        return client.runDiagnostics(projectId)}, [client]);

const _applyQuickFix = useCallback(async (file: string, line: number,;
  fixIndex: number) => {
    if (!client) {t}hrow new Error('Client not initialized');
        return client.applyQuickFix(file, line, fixIndex)}, [client]);
  // Debugging methods;

const _startDebugSession = useCallback(async (config) => {
    if (!client) {t}hrow new Error('Client not initialized'); const _session = await client.startDebugSession(config, setDebugSessions(prev => [...prev, session]);
    return session
}, [client]);

const _setBreakpoint = useCallback(async (file: string, line: number, condition? null : string) => {
    if (!client) {t}hrow new Error('Client not initialized');
        return client.setBreakpoint(file, line, condition)}, [client]);

const _stepOver = useCallback(async (sessionId: string) => {
    if (!client) {t}hrow new Error('Client not initialized');
        return client.stepOver(sessionId)}, [client]);

const _stepInto = useCallback(async (sessionId: string) => {
    if (!client) {t}hrow new Error('Client not initialized');
        return client.stepInto(sessionId)}, [client]);

const _stepOut = useCallback(async (sessionId: string) => {
    if (!client) {t}hrow new Error('Client not initialized');
        return client.stepOut(sessionId)}, [client]);

const _continue_ = useCallback(async (sessionId: string) => {
    if (!client) {t}hrow new Error('Client not initialized');
        return client.continue(sessionId)}, [client]);

const _stopDebugSession = useCallback(async (sessionId: string) =>  {
    if (!client) {t}hrow new Error('Client not initialized', await client.stopDebugSession(sessionId), setDebugSessions(prev => prev.filter((s) => s.id !== sessionId))
}, [client])
  return {
    // Connection
    connected,
