...
export interface UseMCPReturn {
  ...
  connectServer: (serverId: string) => Promise<any >,</any>
  disconnectServer: (serverId: string) => Promise<any >,</any>
  registerCustomServer: (server: Omit<MCPServe
r, 'status' | 'tools'>) => Promise<any></any>
  // Tool operations, tools: MCPTool[],
  callTool: (call: MCPToolCall) => Promise<MCPToolResult></MCPToolResult>
...
n, executePlan: (plan: MCPOrchestrationPlan) => Promise<Map<string MCPToolResult>></Map>
  ...
  listPrompts: (serverId: string) => Promise<any[];>,</any>
  getPrompt: (serverId: string;
  name: string, args?) => Promise<string></string>
...
const [tools, setTools]  = useState<MCPTool[]>([]);</MCPTool>
...
if (options.autoConnect && options.autoConnect.length > 0) { autoConnectServers(options.autoConnect)}
    return () =>  {
...
const serverMap = orchestrator.servers as Map<string MCPServer></string>
...
const _callTool = useCallback(async (call: MCPToolCall): Promise<MCPToolResult> => {</MCPToolResult>
    if (!orchestratorRef.current) {
      throw new Error('Orchestrator not initialized') };
    setLoading(true);
    setError(null);
...
const _callToolsParallel = useCallback(async (calls: MCPToolCall[]): Promise<MCPToolResult[]> => {</MCPToolResult>
    if (!orchestratorRef.current) {
      throw new Error('Orchestrator not initialized') };
    setLoading(true);
    setError(null);
...
const _listResources = useCallback(async (serverId: string): Promise<any[]> => {</any>
    if (!orchestratorRef.current) {
      throw new Error('Orchestrator not initialized') };
    try {
...
const _readResource = useCallback(async (serverId: string, uri: string): Promise<any> => {</any>
    if (!orchestratorRef.current) {
      throw new Error('Orchestrator not initialized')};
    try {
...
const _listPrompts = useCallback(async (serverId: string): Promise<any[]> => {</any>
    if (!orchestratorRef.current) {
      throw new Error('Orchestrator not initialized')};
    try {
...
const _getPrompt = useCallback(async (serverId: string, name: string, args?): Promise<string> => {</string>
    if (!orchestratorRef.current) {
      throw new Error('Orchestrator not initialized')};
    try {
