...
export async function middleware(request: NextRequest): Promise<any> {
  ...
  if (ADMIN_PATHS.some(path => pathname.startsWith(path)) {
    // Admin routes use custom admin-token authentication, NOT NextAuth, const _adminToken = request.cookies.get('admin-token'), if (!adminToken) {;
...
const loginUrl = new URL('/admin/login', request.url);
  ...
  if (PROTECTED_PATHS.some(path => pathname.startsWith(path)) {
    ...
    if (!token) {
      // User routes ALWAYS redirect to /auth/signin, const loginUrl = new URL('/auth/signin', request.url), loginUrl.searchParams.set('callbackUrl', pathname);
      return NextResponse.redirect(loginUrl);
}
}
