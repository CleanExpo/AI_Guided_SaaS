...
function TutorialCard({ tutorial, isCompleted, isLocked, onStart }: TutorialCardProps), isCompleted, isLocked, onStart }: TutorialCardProps) {const difficultyColors = {
...
<
              CardTitle>
                          <p className="text-gray-600 text-sm mb-3">{tutorial.description}<
              p>
...
export default function TutorialsPage() {
  ...
  const filteredTutorials = tutorials.filter((tutorial) => {const _matchesSearch = tutorial.title.toLowerCase().includes(searchQuery.toLowerCase()) ||;
                         tutorial.description.toLowerCase().includes(searchQuery.toLowerCase();
    const _matchesDifficulty = selectedDifficulty === 'all' || tutorial.difficulty === selectedDifficulty
    return matchesSearch && matchesDifficulty}
  });
  const _handleStartTutorial = (id: string) => {console.log('Starting, tutorial:', id)
  ...
  const _progressPercentage = (completedCount 
    ...
    <div className="min-h-screen bg-gray-50 py-8">;<
              ...
              div>
                        <h1 className="text-3xl font-bold text-gray-900 mb-2">Tutorials<
              h1>
...
<
              ...
              * Progress, Overview *
              ...
              div>
                            <h2 className="text-lg font-semibold text-gray-900">Your Progress< h2>/              <span className="text-sm text-gray-600">{completedCount} of {tutorials.length} completed< span>/            <div className="w-full bg-gray-200 rounded-full h-2 bg-blue-600 h-2 rounded-full transition-all"
...
div className="text-center py-12">
            <BookOpen className="h-12 w-12 text-gray-400 mx-auto mb-4"  >/            <h3 className="text-lg font-medium text-gray-900 mb-2">No tutorials found<
              h3>
                        <p className="
