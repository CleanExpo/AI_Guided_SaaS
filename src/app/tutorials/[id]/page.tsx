...
export default function TutorialPage({ params }: { params: { id: string }}): { params: { id: string }}) {const tutorial = tutorials[params.id]
  if(!tutorial) {
    notFound()}
  return (
    <div className="min-h-screen bg-gray-50 py-8 container mx-auto px-4 max-w-4xl">;<
                      ...
                      <div className="bg-white rounded-lg shadow-sm p-8">
          {
              * Header *
              ...
              span>
                            <span className="text-gray-600">{tutorial.duration}<
              span>
                          <h1 className="text-3xl font-bold text-gray-900 mb-4">{tutorial.title}<
              h1>
                          <p className="text-xl text-gray-600">{tutorial.description}<
              p>
                        {
              * Prerequisites *
              }, {tutorial.prerequisites.length > 0  && (div className="mb-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">/              <h3 className="font-semibold text-gray-900 mb-2">Prerequisites< h3>/              <ul className="list-disc list-inside space-y-1">}
                {tutorial.prerequisites.map((prereq, index) => (\n    <li key={index} className="text-gray-700">{prereq}<
              li>
                              ))}
<
              ul>
