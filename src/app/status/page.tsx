...
const _getStatusColor = (status: string) => {switch (status) {
    case 'operational':;
      return 'bg-green-100 text-green-800', break, case 'maintenance':;
...
export default function StatusPage() {
  const _overallStatus = services.some(s => s.status === 'outage') ? 'outage' :, services.some(s => s.status === 'degraded') ? 'degraded' :, services.some(s => s.status === 'maintenance') ? 'maintenance' : 'operational';
  return (
  ...
  return (getStatusIcon(overallStatus)}</div>
                          <span className="ml-2 text-lg font-medium text-gray-900">
              All Systems {overallStatus === 'operational' ? 'Operational' :, overallStatus === 'maintenance' ? 'Under Maintenance' :}
                         overallStatus === 'degraded' ? 'Degraded' : 'Down'}</span>
