...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    const formData = await request.formData(); const file = formData.get('file') as File, if (!file) {
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {;
    const url = new URL(request.url); const uploadId = url.searchParams.get('uploadId'), if (!uploadId) {
