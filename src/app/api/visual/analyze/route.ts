...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    const body = await request.json(); const { imageUrl, analysisType = 'general' }: any = body;
    if (!imageUrl) {
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {;
    const url = new URL(request.url); const analysisId = url.searchParams.get('analysisId'), if (!analysisId) {
