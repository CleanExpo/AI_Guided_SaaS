...
const indexSchema = z.object({ id: z.string().min(1, content: z.string().min(1, metadata: z.record(z.any()).optional(, type: z.enum(['document', 'code', 'log', 'config', 'memory', 'conversation']).optional()
});
...
export async function DELETE(request: NextRequest): Promise<NextResponse> {</NextResponse>
  try {
