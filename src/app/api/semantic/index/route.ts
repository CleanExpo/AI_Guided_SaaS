...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    const body = await request.json(), // Check if it's a batch request, if (Array.isArray(body)) {;
      // Batch indexing;
...
const results = await semanticSearch.indexBatch(validatedData);
      ...
      // Single document indexing, const validatedData  = indexSchema.parse(body); const result = await semanticSearch.indexDocument(validatedData);
