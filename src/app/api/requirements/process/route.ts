...
const processSchema = z.object({ requirements: z.string().min(1, 'Requirements are required', projectType: z.enum(['web-app', 'api', 'mobile', 'desktop']),
...
export async function POST(request: NextRequest): Promise<NextResponse> {</NextResponse>
    try {
        ...
        const processed={ id: 'req_' + Math.random().toString(36).substr(2, 9, ...validatedData,
            status: 'processed',
