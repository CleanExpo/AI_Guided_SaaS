...
const chatSchema = z.object({ message: z.string().min(1, 'Message is required', context: z.record(z.any()).optional(, sessionId: z.string().optional()
})
export async function POST(request: NextRequest): Promise<NextResponse> {</NextResponse>
  try {
    ...
    const response={ id: 'msg_' + Math.random().toString(36).substr(2, 9, message: `I understand you're asking about: "${validatedData.message}". Here's how I can help...`,
      suggestions: [
        'Check our documentation',
        'Contact technical support',
        'View video tutorials'
      ],
      sessionId: validatedData.sessionId || 'session_' + Math.random().toString(36).substr(2, 9, timestamp: new Date().toISOString()
};
