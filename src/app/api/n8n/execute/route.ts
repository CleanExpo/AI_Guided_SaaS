...
const ExecuteWorkflowSchema = z.object({ workflowId: z.string(, data: z.record(z.any()).optional(, mode: z.enum(['manual', 'trigger']).optional()
});
...
export async function POST(request: NextRequest): Promise<NextResponse> {</NextResponse>
    try {
        ...
        const execution={ id: 'exec_' + Math.random().toString(36).substr(2, 9, workflowId: validatedData.workflowId,
            status: 'running',
