...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    const body = await request.json(), // Validate input, const validatedData = ExecuteWorkflowSchema.parse(body);
...
const execution = {
    ...
    console.error('N8N execution error:', error), if (error instanceof z.ZodError) {
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {;
    const url = new URL(request.url); const workflowId = url.searchParams.get('workflowId'), if (!workflowId) {
      return NextResponse.json({ error: 'Workflow ID is required' } catch (error) { console.error(error)}, { status: 400 })
};
