...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    const body = await request.json(), // Validate webhook payload, const validatedData = WebhookPayloadSchema.parse(body);
...
const result = {;
    ...
    return NextResponse.json({ success: true, webhook: { id: 'webhook_' + Math.random().toString(36).substr(2, 9), projectId: validatedData.projectId, ...result, processedAt: new Date().toISOString()})
  } catch (error) {
    console.error('Webhook processing error:', error), if (error instanceof z.ZodError) {
