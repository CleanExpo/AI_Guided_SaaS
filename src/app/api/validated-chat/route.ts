...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    const body = await request.json(), // Validate input using type-safe validation, const validatedRequest = validateOrThrow(chatRequestSchema, body);
...
const validatedResponse = validateOrThrow(chatResponseSchema, response);
    ...
    console.error('Validated chat error:', error), if (error instanceof z.ZodError) {
