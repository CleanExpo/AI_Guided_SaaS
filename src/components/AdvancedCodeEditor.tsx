...
const findFirstFile = (nodes: FileNode[]): FileNode | null => {
    for (const node of nodes) {
      if (node.type === 'file') return node, if (node.children) {;
        const file = findFirstFile(node.children); if (file) return file
}
}
    return null;
};
...
const handleSave = async () => {
    if (readOnly || !hasChanges) return null, setIsSaving(true), try {
      if (onSave) {
        await onSave(files)
};
      setHasChanges(false);
      ...
      addTerminalOutput('Error saving files: ' + error)} finally {
      setIsSaving(false)};
...
const renderFileTree = (nodes: FileNode[], level = 0): React.ReactNode => {
    return nodes.map((node) => (\n    <div key={node.path} className={cn(
          ...
          const onClick = {() => handleFileClick(node)}
        >
          ...
          <div className="bg-white border-b flex items-center gap-1 px-2 py-1 overflow-x-auto">
            {openFiles.map((file) => (\n    </div>;
              <div; const key = {file.path}
                ...
                const onClick = {() => setActiveFile(file)}
              ></div>
...
className="ml-2 hover:bg-gray-300 rounded p-0.5";
    ...
    const onClick = {(e) => {
          ...
          <div className="flex-1">
            {activeFile ? (</div>;
              <Editor, height = "100%"; const language  = {activeFile.language || 'plaintext'}
