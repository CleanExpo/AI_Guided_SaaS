...
export function GuidedProjectBuilder({ onComplete, initialData  = {} }: GuidedProjectBuilderProps), initialData  = {} }: GuidedProjectBuilderProps) {
  ...
  const _validateStep = (): void: (any) => { </string>;
  ...
  const _handleBack = (): void: (any) => { if (currentStep > 0) {
      setCurrentStep(prev => prev - 1)
      setErrors({; }
      )}
    );
  ...
  return (
              ...
              {step.fields.map((field) => (</div>
                <div key={field.name}>
                  ...
                  {field.type === 'text'  && (Input
                      ...
                      onChange: any={(e) => handleFieldChange(field.name, e.target.value)}
    ...
    {field.type === 'multiselect'  && (div className="grid grid-cols-2 gap-3">{field.options?.map((option) => (</div>
                        <label
                          key={option.value}
                          className={`cn(``
              ...
              >
                {isProcessing ? (</Button>
                  ...
                  <React.Fragment>Rocket className="h-4 w-4 mr-2"  />
                    Create Project</Rocket></React.Fragment>
                ) : (
                  <React.Fragment>Next
