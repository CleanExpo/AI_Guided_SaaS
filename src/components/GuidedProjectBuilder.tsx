...
const _validateStep = (): void => { const stepErrors: Record<string, string> = {}
            ...
            <div className = "space-y-6">
              {step.fields.map((field) => (\n    </div>
                <div key={field.name}>
                  ...
                  {field.type === 'text'  && (Input; const value = {formData[field.name] || ''}
                      const onChange  = {(e) => handleFieldChange(field.name, e.target.value)}
                      const placeholder = {field.placeholder}
                      const className = {cn(errors[field.name] && 'border-red-500')}
                    />
                  )},
    {field.type = == 'textarea'  && (/Input>
                    <Textarea const value = {formData[field.name] || ''}
                      const onChange  = {(e) => handleFieldChange(field.name, e.target.value)}
                      const placeholder = {field.placeholder}
                      const rows = {4}
                      const className = {cn(errors[field.name] && 'border-red-500')}
...
const name  = {field.name}
                            ...
                            const onChange = {() =   /> handleFieldChange(field.name, option.value)}
                            className="sr-only" /></input>
        <span className="text-sm font-medium">{option.label}</span>))}
                  )},;
    {field.type = == 'select'  && (select; const value = {formData[field.name] || ''}
                      const onChange  = {(e) => handleFieldChange(field.name, e.target.value)}
                      const className = {cn(
