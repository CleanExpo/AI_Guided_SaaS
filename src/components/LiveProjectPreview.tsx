...
switch (projectType) { case 'website':
        ...
        ${features.includes('analytics') ? `
          <section className="py-16 px-6 bg-gray-50">
            <div className="container mx-auto">
              ...
              <div className="grid grid-cols-3 gap-6" >className="bg-white p-6 rounded-lg shadow"></div>
  ...
  const _generateWebAppTemplate = (): void => {;
    ...
    return ```;
      <div className="${designStyle} h-full">
        <!-- App Bar -->
        <div className="${primaryColor} text-white p-4 flex items-center justify-between" >className="flex items-center gap-4"></div>
            <div className="w-10 h-10 bg-white/20 rounded-lg"   />
            <h1 className="text-xl font-semibold">${projectName || 'Your, App'}</h1>
          <div className="flex items-center gap-4">
            ${features.includes('notifications') ? '<div className="relative" >className="w-8 h-8 bg-white/20 rounded-full"></div><div className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full">' : ''}</div>
        ...
        <div className="flex h-full">
          ...
          <div className="flex-1 p-6">
            ${features.includes('realtime') ? `</div>``
              <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg" >className="flex items-center gap-2"></div>
                  <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"   />
                  <span className="text-sm text-green-700">Real-time updates enabled</span>
            ` : ''}``
            <div className="grid grid-cols-2 gap-6" >className="bg-white p-6 rounded-lg shadow"></div>
                <h3 className="text-lg font-semibold mb-4">Recent Activity</h3>
                <div className="space-y-3" >className="flex items-center gap-3"></div>
              ...
              ${features.includes('analytics') ? ```
                <div className="bg-white p-6 rounded-lg shadow">
                  ...
                  <div className="space-y-3" >className="flex justify-between"></div>
      ...
      <div className="${designStyle}">
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-3 gap-6">
            ${[1, 2, 3].map((i) => `</div>``
              <div className="bg-white rounded-lg shadow-lg overflow-hidden" >className="h-48 bg-gray-200"></div>
                <div className="p-4">
        ...
        ${features.includes('payments') ? ```
          <div className="container mx-auto px-4 py-4 border-t" >className="flex items-center justify-center gap-4 text-gray-500"></div>
              <span className="text-sm">Secure payments with</span>
              <div className="flex gap-2" >className="w-12 h-8 bg-gray-200 rounded"></div>
  ...
  const _generateDashboardTemplate = (): void => {;
    ...
    return ```;
      <div className="${designStyle} bg-gray-50 h-full">
        <!-- Top Bar -->
        <div className="bg-white border-b px-6 py-4" >className="flex items-center justify-between"></div>
            <h1 className="text-xl font-bold">${projectName || 'Dashboard'}</h1>
            <div className="flex items-center gap-4">
              ${features.includes('notifications') ? '<div className="relative" >className="w-8 h-8 bg-gray-200 rounded-full"></div><div className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full">' : ''}</div>
              <div className="w-8 h-8 bg-gray-200 rounded-full"   />
        <!-- Dashboard Content -->
        <div className="p-6">
          <!-- Stats Cards -->
  ...
  const _generateDefaultTemplate = (): void => {
    return ```, <div, className="${designStyle} p-8 text-center" >className="mb-8"></div>
