...
export function LiveProjectPreview({ projectData, mockData }: LiveProjectPreviewProps), mockData }: LiveProjectPreviewProps) {</string>;
...
return generateEcommerceTemplate();
        ...
        ${features.includes('analytics') ? `
          <section class="py-16 px-6 bg-gray-50">
            <div class="container mx-auto">
              ...
              <div class="grid grid-cols-3 gap-6" >class="bg-white p-6 rounded-lg shadow"></div>
                  <div class="text-3xl font-bold ${primaryColor.replace('bg-', 'text-')}">1,234</div>
                  <div class="text-gray-600">Total Visitors</div>
                <div class="bg-white p-6 rounded-lg shadow" >class="text-3xl font-bold ${primaryColor.replace('bg-', 'text-')}">567</div>
                  <div class="text-gray-600">Active Users</div>
                <div class="bg-white p-6 rounded-lg shadow" >class="text-3xl font-bold ${primaryColor.replace('bg-', 'text-')}">89%</div>
  ...
  const _generateWebAppTemplate = (): void => {
    ...
    return ```;
      <div class="${designStyle} h-full">
        <!-- App Bar -->
        <div class="${primaryColor} text-white p-4 flex items-center justify-between" >class="flex items-center gap-4"></div>
            <div class="w-10 h-10 bg-white/20 rounded-lg"></div>
            <h1 class="text-xl font-semibold">${projectName || 'Your, App'}</h1>
          <div class="flex items-center gap-4">
            ${features.includes('notifications') ? '<div class="relative" >class="w-8 h-8 bg-white/20 rounded-full"></div><div class="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full">' : ''}</div>
        ...
        <div class="flex h-full">
          ...
          <div class="flex-1 p-6">
            ${features.includes('realtime') ? `</div>``
              <div class="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg" >class="flex items-center gap-2"></div>
                  <div class="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                  <span class="text-sm text-green-700">Real-time updates enabled</span>
            ` : ''}``
            <div class="grid grid-cols-2 gap-6" >class="bg-white p-6 rounded-lg shadow"></div>
                <h3 class="text-lg font-semibold mb-4">Recent Activity</h3>
                <div class="space-y-3" >class="flex items-center gap-3"></div>
              ...
              ${features.includes('analytics') ? ```
                <div class="bg-white p-6 rounded-lg shadow">
                  ...
                  <div class="space-y-3" >class="flex justify-between"></div>
    ...
    return ```;
      <div class="${designStyle}">
        ...
        <header class="border-b">
          <div class="container mx-auto px-4 py-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-6">
              ...
              <div class="relative" >class="w-8 h-8 ${primaryColor} text-white rounded-full flex items-center justify-center text-sm">3</div>
        <!-- Products Grid -->
        <div class="container mx-auto px-4 py-8">
          <h2 class="text-2xl font-bold mb-6">Featured Products</h2>
          <div class="grid grid-cols-3 gap-6">
            ${[1, 2, 3].map((i) => `</div>``
              <div class="bg-white rounded-lg shadow-lg overflow-hidden" >class="h-48 bg-gray-200"></div>
                <div class="p-4">
        ...
        ${features.includes('payments') ? ```
          <div class="container mx-auto px-4 py-4 border-t" >class="flex items-center justify-center gap-4 text-gray-500"></div>
              <span class="text-sm">Secure payments with</span>
              <div class="flex gap-2" >class="w-12 h-8 bg-gray-200 rounded"></div>
  ...
  const _generateDashboardTemplate = (): void => {
    ...
    return ```;
      <div class="${designStyle} bg-gray-50 h-full">
        <!-- Top Bar -->
        <div class="bg-white border-b px-6 py-4" >class="flex items-center justify-between"></div>
            <h1 class="text-xl font-bold">${projectName || 'Dashboard'}</h1>
            <div class="flex items-center gap-4">
              ${features.includes('notifications') ? '<div class="relative" >class="w-8 h-8 bg-gray-200 rounded-full"></div><div class="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full">' : ''}</div>
              <div class="w-8 h-8 bg-gray-200 rounded-full"></div>
        <!-- Dashboard Content -->
        <div class="p-6">
          <!-- Stats Cards -->
          <div class="grid grid-cols-4 gap-6 mb-6">
            ${['Total Revenue', 'Active Users', 'Conversion Rate', 'Growth'].map((label, i) => `</div>``
              <div class="bg-white p-6 rounded-lg shadow" >class="text-sm text-gray-600 mb-2">${label}</div>
  ...
  const _generateDefaultTemplate = (): void => {
    return ```
      <div, class="${designStyle} p-8 text-center" >class="mb-8"></div>
  ...
  return (
    <Card className="h-full flex flex-col">
      <div className="p-4 border-b">
        <div className="flex items-center justify-between mb-4 flex items-center gap-2"></div>
...
Badge variant="secondary", className="animate-pulse">
      ...
      <div className="flex-1 bg-gray-100 p-4 overflow-auto" className={cn(``
        ...
        <div className="">
        <p className="Generating preview..."></p>
        </div>
    ); : (
            <div
