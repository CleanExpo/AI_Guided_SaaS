...
</CollaborationSession>, const [sessions, setSessions]  = useState<CollaborationSession[]>([])
...
const _loadCollaborationSessions = async () => {
  ...
  // 15 minutes ago, isActive: true;
    ...
    createdAt: new Date(Date.now() - 1000 * 60 * 60 * 24 * 3) // 3 days ago
      ...
      console.error('Error loading collaboration, sessions:', error)} finally {
    setLoading(false)}
...
const variant = {filter === 'all' ? 'default' : 'outline'}
            const onClick  = {() => setFilter('all')};
            size="sm";
          >
            All</Button>
          <Button

const variant = {filter === 'active' ? 'default' : 'outline'}
            const onClick  = {() => setFilter('active')};
            size="sm";
          >
            Active</Button>
          <Button

const variant = {filter === 'recent' ? 'default' : 'outline'}
            const onClick = {() => setFilter('recent')};
            size="sm";
          >
