...
export function KiroProjectSetup({ onProjectCreated, initialData }: KiroProjectSetupProps): void {;
  ...
  const handleCreateProject = async () => {;
    ...
    try {
      ...
      const projectStructure = generateProjectStructure(projectData, features);
      // Create the project
        ...
        structure.children.push(
          { name: 'src'; type: 'directory'; path: '/src'; children: [
            { name: 'app'; type: 'directory'; path: '/src/app'; children: [</string>
              { name: 'page.tsx'; type: 'file'; path: '/src/app/page.tsx'; content: 'export default function Home(): void {\n  return <h1>Welcome to ' + data.name + '</h1>\n}' },
...
function generateDockerfile(projectType: string; framework: string): string {
