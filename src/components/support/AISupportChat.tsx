...
if (!isOpen) {
    return (<Button, className = "fixed bottom-6 right-6 h-14 w-14 rounded-full shadow-lg z-50"; const onClick  = {() => setIsOpen(true)}
      ></Button>
...
const value  = {searchQuery}
    ...
    {message.metadata?.suggestedDocs && message.metadata.suggestedDocs.length > 0  && (div className="max-w-[85%] space-y-1">
        ...
        return doc ? (<button, const key = {docId};
                                const onClick = {() => handleActionButton('open-doc', { sectionId: docId})};
...
const onClick  = {() => handleActionButton(button.action, button.data)};
                              className="text-xs";
                            >
                              {button.label}</Button>
                          ))})})},
...
const onClick  = {() => handleQuickAction(action.action)}
                    ></Button>
