...
export default function FlowchartBuilder({ projectName, onSaveFlow }: FlowchartBuilderProps), onSaveFlow }: FlowchartBuilderProps) {
  ...
  const [edges, setEdges, onEdgesChange] = useEdgesState(initialEdges), const [nodeId, setNodeId]  = useState<any>(5);
...
const _importFlow = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    const _file = event.target.files?.[0], if (file) {;
