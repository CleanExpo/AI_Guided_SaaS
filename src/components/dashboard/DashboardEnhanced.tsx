...
SimpleChart({ data }: { data }): { data }) {;
  const _maxValue = Math.max(...data.map((d) => d.users);
        return (<div className="flex items-end justify-between h-32 gap-2">, {data.map((item, index) => (\n    </div>;
        <motion.div;
...
const key = {activity.id}
              ...
              const transition = {{ duration: 0.3 delay: index * 0.1 };
...
const icon  = {<Settings className="h-4 w-4"   />}
            /></ButtonPremium>
      {/* Metrics, Grid */}
      <div className="grid grid-cols-1, md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {metrics.map((metric, index) => (\n    </div>
          <MetricCard, const key = {metric.title}
