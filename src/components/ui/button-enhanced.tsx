...
export function IconButton({
  ...
  return (<ButtonEnhanced className={cn('p-0', className)} size="icon" {...props}>
      {icon}
</ButtonEnhanced>
      )}
// Toggle Button
...
export function ToggleButton({
  ...
  return (ButtonEnhanced, className={cn(pressed && 'bg-accent text-accent-foreground', className)}
      const variant = {pressed ? 'default' : 'outline'}
      const onClick = {() => onPressedChange?.(!pressed)}
...
export function SplitButton({
  ...
  return (<div className="relative inline-flex">, <ButtonEnhanced
...
const variant = {props.variant}
        ...
        const onClick = {() => onDropdownToggle?.(!dropdownOpen)}
...
const onClick = {handleCopy}
      const variant = {copied ? 'success' : 'outline'}
      {...props}
    >
