...
export function ButtonEnhanced({
  ...
  return (button, className={cn(
    ...
    >
      ...
      {!loading && leftIcon && <span className="mr-2">{leftIcon}}
      {children}
      {!loading && rightIcon && <span className="ml-2">{rightIcon}}
...
export function SplitButton({;
  ...
  return ()
    <div className="relative inline-flex">, <ButtonEnhanced className={cn('rounded-r-none border-r-0', className)}>{...props}>
        {children}

      <ButtonEnhanced className="rounded-lg-l-none px-2";
>variant={props.variant} size={props.size}>const onClick={() => onDropdownToggle?.(!dropdownOpen)}
        ...
        <div className="absolute top-full left-0 mt-1 w-full min-w-48 bg-background  rounded-lg-md shadow-md-lg z-50">
          {dropdownItems.map((item, index) => (\n    
            <button const key={index};
              className="w-full px-3 py-2 text-left text-sm hover: bg-accent hover:text-accent-foreground disabled:opacity-50 disabled:cursor-not-allowed flex items-center">onClick={ item.onClick} disabled={item.disabled}>aria-label="Button">
              {item.icon && <span className="mr-2">{item.icon}}
              {item.label}
...
onClick={handleCopy} variant={copied ? 'success' : 'outline'}>{...props}>
...
const config = providerConfig[provider];
  return (<ButtonEnhanced variant="outline";
    ...
    const leftIcon={config.icon})
      className={cn('w-full', config.className, className)}>{...props}>
      {children ||
