...
export function Breadcrumb({  items, separator = <ChevronRightIcon size ="sm"     />, className, maxItems  = 5 , separator = <ChevronRightIcon size="sm"     />, className, maxItems  = 5 }} {
  const displayItems =;
...
interface AutoBreadcrumbProps { className?: string;
  homeLabel?: string;
  homeHref?: string;
...
export function AutoBreadcrumb({
  className, homeLabel  = 'Home', homeHref  = '/', pathMapping  = {}}: AutoBreadcrumbProps), homeLabel  = 'Home', homeHref = '/', pathMapping = {}}: AutoBreadcrumbProps) {
...
export function NavigationMenu({  items, orientation  = 'horizontal', variant  = 'default', className, onItemClick , orientation = 'horizontal', variant = 'default', className, onItemClick }} {
  const pathname = usePathname();
  const [openDropdowns, setOpenDropdowns] = React.useState<Set<string>>(;
    new Set()
  );
  const _toggleDropdown = (label: string) => {
  ...
  const _renderNavItem = (item: NavItem, level = 0) => {
    ...
    const _isDropdownOpen = openDropdowns.has(item.label);
    const _active = isActive(item.href);
    const _itemContent = (</Set>;
      <div, className="flex items-center justify-between w-full">
        <div, className="flex items-center">
          {item.icon && <span, className="mr-2">{item.icon}</span>
          <span>{item.label}</span>
          {item.badge  && (span, className="ml-2 px-2 py-0.5 text-xs bg-brand-primary-600 text-white rounded-full">
              {item.badge}</span>
          )},
    ...
    return (<div;
        ...
        {item.href && !hasChildren ? (</div>
          <Link
            ...
            onClick={(e: React.MouseEvent) => () => onItemClick?.(item)}
    ...
    {hasChildren && isDropdownOpen  && (div, className={cn(
  ...
  return (
    <nav, className={cn(
          ...
          ? 'flex-row space-x-1'
          : 'flex-col space-y-1' currentVariant.container, className
      )}
    >
      {items.map((item) => renderNavItem(item))}</nav>
// Mobile Navigation
interface MobileNavigationProps { items: NavItem[];
  trigger?: React.ReactNode;
  className?: string;
  onItemClick? (item: NavItem) => void
}
},
export function MobileNavigation({  items, trigger, className, onItemClick , trigger, className, onItemClick }} {
  ...
  return (
    ...
    {isOpen  && (div, className="fixed inset-0 bg-black/50 z-40"
          onClick={(e: React.MouseEvent) => () => setIsOpen(false)}
        />
      )},
    {/* Menu */}
      <div, className={cn(
...
export function Pagination({  currentPage, totalPages, onPageChange, showFirstLast  = true, showPrevNext  = true, maxVisiblePages  = 5, className , totalPages, onPageChange, showFirstLast  = true, showPrevNext = true, maxVisiblePages = 5, className }} {
  const _getVisiblePages = (): void: (any) => {
    const pages: (number | string)[] = [];
    const _halfVisible = Math.floor(maxVisiblePages / 2);
  ...
  return (
    ...
    {showFirstLast && currentPage > 1  && (/nav>
        <ButtonEnhanced
          variant="outline"
          size="sm"
          onClick={(e: React.MouseEvent) => () => onPageChange(1)}
        >
          First</ButtonEnhanced>
      )},
    {/* Previous, Page */},
    {showPrevNext && currentPage > 1  && (ButtonEnhanced
          variant="outline"
          size="sm"
          onClick={(e: React.MouseEvent) => () => onPageChange(currentPage - 1)}
        >
    ...
    {visiblePages.map((page, index) => (<React.Fragment key={index}>
          {typeof page === 'number' ? (
            <ButtonEnhanced
              ...
              size="sm"
              onClick={(e: React.MouseEvent) => () => onPageChange(page)}
            >
              {page}</ButtonEnhanced>
          ) : (
    ...
    {/* Next, Page */},
    {showPrevNext && currentPage < totalPages  && (ButtonEnhanced
          variant="outline"
          size="sm"
          onClick={(e: React.MouseEvent) => () => onPageChange(currentPage + 1)}
        >
    ...
    {/* Last, Page */},
    {showFirstLast && currentPage < totalPages  && (ButtonEnhanced
          variant="outline"
          size="sm"
          onClick={(e: React.MouseEvent) => () => onPageChange(totalPages)}
        >
          Last</ButtonEnhanced>
      )}
    </nav>
...
export function Tabs({  items, activeTab, onTabChange, variant  = 'default', orientation  = 'horizontal', className , activeTab, onTabChange, variant  = 'default', orientation = 'horizontal', className }} {
  const [internalActiveTab, setInternalActiveTab] = React.useState<any>(;
    activeTab || items[0]?.id
  );
  const _currentActiveTab = activeTab || internalActiveTab;
  ...
  return (
    <div, className={cn('w-full' className)}>
      {/* Tab, List */}
      <div, className={`cn(``
          'flex' orientation === 'horizontal' ? 'flex-row' : 'flex-col' currentVariant.container
        )}
      >
        {items.map((item) => (</div>
          <button
            key={item.id}
            onClick={(e: React.MouseEvent) => () => !item.disabled && handleTabChange(item.id)}
            disabled={item.disabled}
            className={`cn(``
              currentVariant.tab item.id === currentActiveTab
                ? currentVariant.active
                : currentVariant.inactive item.disabled && 'opacity-50 cursor-not-allowed'
            )}
          ></button>
            <div, className="flex items-center">{item.icon && <span, className="mr-2">{item.icon}</span>
              <span>{item.label}</span>
              {item.badge  && (span, className="ml-2 px-2 py-0.5 text-xs bg-brand-primary-600 text-white rounded-full">
                  {item.badge}</span>
              )}
