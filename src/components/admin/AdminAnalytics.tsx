...
export function AdminAnalytics({ data, timeRange }: AdminAnalyticsProps) {
  ...
  const SimpleLineChart = ({ data, height = 200 }: { data: Array<{ date: string count: number }>; height?: number }) => {
    ...
    return (
    <div className="relative" style={{ height }} className="absolute inset-0 flex items-end justify-between gap-1">
    ...
    <div
               ...
               className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-slate-900 text-white text-xs rounded px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                  {point.count.toLocaleString()}</div>
      )}
)}
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="mt-6 space-y-2 flex justify-between items-center"></div>
                  <span className="text-sm text-muted-foreground">Peak Users</span>
                  <span className="font-medium">
                    {Math.max(...data.userMetrics.activeUsers.map((d) => d.count)).toLocaleString()}</span>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">Average Users</span>
                  <span className="font-medium">
                    {Math.round(data.userMetrics.activeUsers.reduce((a, b) => a + b.count, 0) / data.userMetrics.activeUsers.length).toLocaleString()}</span>
</CardContent>
</div>
      )}
      {/* Project Metrics */}
            ...
            <CardContent>
              <div className="space-y-4">
                {data.projectMetrics.projectTypes.map((type) => (\n    </div>
                  <div key={type.type} className="flex justify-between items-center mb-1"></div>
                  ...
                  <div className="space-y-3">
                    {data.apiMetrics.topEndpoints.map((endpoint) => (\n    </div>
                      <div key={endpoint.endpoint} className="flex items-center justify-between" >
                        ...
                        <div className="text-right w-32"></div>
                            <Progress
      ...
      {selectedMetric === 'revenue' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4 space-y-2"></div>
                    <p className="text-sm text-muted-foreground">MRR</p>
                    <p className="text-2xl font-bold">{formatCurrency(data.overview.revenue / 12
            )}</p>
                  <div className="">
        <p className="text-sm text-muted-foreground">ARR</p>
                    <p className="text-2xl font-bold">{formatCurrency(data.overview.revenue)}</p>
                <div className="pt-4 flex justify-between items-center mb-2"></div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="space-y-2 flex justify-between items-center"></div>
                ...
                <div className="pt-4 border-t flex justify-between items-center"></div>
