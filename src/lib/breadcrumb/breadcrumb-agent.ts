...
break;
  ...
  private checkFeatureCoverage(result: ValidationResult) {
    ...
    for(const module of this.vision.modules) {
      const _hasImplementation = Object.values(this.index.files).some(mapping =>;
  ...
  getBreadcrumbTrail(filePath: string): string[] {
    ...
    if (!mapping) return [`⚠️ ${filePath} is not mapped to project vision`];``// Build trail from vision to file
