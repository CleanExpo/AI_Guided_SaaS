...
class CycleDetectionEngine {;
   ...
   */
  recordAttempt(attempt: Omit<ProblemAttempt, 'id' | 'timestamp' | 'contextHash'>): string {
    const id = this.generateId();
    const contextHash = this.generateContextHash(attempt);
  ...
  detectCycle(sessionId: string): CycleDetectionResult {
    ...
    const recentAttempts = this.getRecentAttempts(sessionAttempts);
    ...
    if (cycleAnalysis.isCyclic) {
      const relevantSources = this.identifyRelevantDocumentation(recentAttempts);
      return {
        ...cycleAnalysis,
   ...
   */
  async searchDocumentationSources(
    problemDescription: string;
    errorMessages: string[];
    relevantSources: string[]
  ): Promise<DocumentationSearchResult[]> {
    const searchQueries = this.generateSearchQueries(problemDescription, errorMessages);
    ...
    for (const sourceName of relevantSources) {
      ...
      if (!source) continue;
      try {
        const searchResult = await this.searchDocumentationSource(source, searchQueries);
        if (searchResult) {
   ...
   */
  generateBreakpointSuggestion(attempts: ProblemAttempt[]): string {
  ...
  private generateContextHash(attempt: Omit<ProblemAttempt, 'id' | 'timestamp' | 'contextHash'>): string {
    const context = `${attempt.problemDescription}|${attempt.attemptedSolution}|${attempt.errorMessages.join('|')}`;`
  ...
  private identifyPatterns(attempts: ProblemAttempt[]): Map<string, number> {
  ...
  private extractPattern(attempt: ProblemAttempt): string {
    // Create a pattern based on problem type, solution type, and error type
    const problemType = this.categorizeText(attempt.problemDescription);
    const solutionType = this.categorizeText(attempt.attemptedSolution);
    const errorType = attempt.errorMessages.map(err => this.categorizeText(err)).join(',');
  ...
  private analyzeCyclicBehavior(patterns: Map<string, number>): Omit<CycleDetectionResult, 'documentationSources' | 'suggestedBreakpoint'> {
    const repeatedPatterns = Array.from(patterns.entries());
      .filter(([, count]) => count >= this.CYCLE_THRESHOLD)
      .map(([pattern]) => pattern);
    const isCyclic = repeatedPatterns.length > 0;
    const cycleLength = patterns.size > 0 ? Math.max(...Array.from(patterns.values())) : 0;
    const confidence = isCyclic ? Math.min(cycleLength / this.CYCLE_THRESHOLD, 1) : 0;
    return {
      isCyclic,
      cycleLength,
      confidence,
  ...
  private identifyRelevantDocumentation(attempts: ProblemAttempt[]): string[] {
    ...
    attempts.forEach(attempt => {
      const problemCategory = this.categorizeText(attempt.problemDescription);
    ...
    const relevantSources = this.documentationSources;
      .filter(source =>
        source.categories.some(cat => categories.has(cat)) ||
  ...
  private async searchDocumentationSource(
    source: DocumentationSource;
    queries: string[]
  ): Promise<DocumentationSearchResult | null> {
    // This would integrate with actual search APIs or web scraping
    // For now, return a structured result format
    const searchUrl = `${source.baseUrl}?q=${encodeURIComponent(queries[0])}`;`
    return {
  ...
  private extractCommonErrors(attempts: ProblemAttempt[]): string[] {
    ...
    attempts.forEach(attempt => {
      attempt.errorMessages.forEach(error => {
        const normalizedError = error.substring(0, 100); // Truncate for grouping
...
// Singleton instance
export const cycleDetectionEngine = new CycleDetectionEngine();
