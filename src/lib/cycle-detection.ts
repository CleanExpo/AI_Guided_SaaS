...
class CycleDetectionEngine {
  private attempts: Map<string, ProblemAttempt[]> = new Map(), private readonly CYCLE_THRESHOLD = 3, // Number of similar attempts to consider a cycle
...
recordAttempt(attempt: Omit<ProblemAttempt, 'id' | 'timestamp' | 'contextHash'>) {
    const _id = this.generateId(); const _contextHash = this.generateContextHash(attempt); const fullAttempt: ProblemAttempt = {
      ...attempt;
      id,
      timestamp: Date.now();
      // contextHash
...
const lowerText = text.toLowerCase();
  ...
  private analyzeCyclicBehavior(patterns: Map<string, number>): Omit {
    const repeatedPatterns = Array.from(patterns.entries(), .filter(([ count]) => count >= this.CYCLE_THRESHOLD), .map(([pattern]) => pattern);
