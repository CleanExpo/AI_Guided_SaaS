...
export class HierarchicalMemorySystem {
  ...
  // 30 days retention, priorityWeight: 0.7,
    ...
    this.projectMemory = {
      name: 'Project Memory',
      maxTokens: 50000;
  // ./CLAUDE.md team-shared standards, currentTokens: 0,
    priority: 'high',
    retentionPolicy: {
  type: 'priority',
    parameters: {,
  priorityWeight: 0.8,
    ...
    this.modularMemory = {
      name: 'Modular Memory',
      maxTokens: 135000;
  // @path/to/import modular files, currentTokens: 0,
    priority: 'medium',
    retentionPolicy: {
  type: 'frequency',
    parameters: {,
  ...
  async updateMemoryEntry(id: string, updates: Partial<MemoryEntry>): Promise<any> {
    ...
    if(updates.metadata?.size && updates.metadata.size !== oldSize) {
      const _sizeDelta = updates.metadata.size - oldSize;
  ...
  private calculateRetentionScore(entry: MemoryEntry, policy: RetentionPolicy): number {
    const now = new Date();
    const _daysSinceCreated = (now.getTime() - entry.metadata.created.getTime()) / (1000 * 60 * 60 * 24);
    const _daysSinceAccessed = (now.getTime() - entry.metadata.lastAccessed.getTime()) / (1000 * 60 * 60 * 24);
    let score = 0;
    // Priority component
    ...
    switch (policy.type) { case 'priority':
