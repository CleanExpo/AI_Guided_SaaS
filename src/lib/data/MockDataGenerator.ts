...
export class MockDataGenerator {
  ...
  private generateFieldValue(field: SchemaField, index: number): any {
    switch (field.type) {
      case 'string':
        if (field.enum) {
  ...
  generateRelatedData(schemas: string[], recordsPerSchema: number = 10): Record<string, any[]> {
    ...
    schemas.forEach(schemaName => {
      ...
      schema.relationships.forEach(rel => {
        ...
        const toData = result[rel.to]
        
        if (!fromData || !toData) return
        
        fromData.forEach(record => {
  ...
  exportData(data: any[], format: 'json' | 'csv' | 'sql' = 'json'): string {
    switch (format) {
      case 'json':
      ...
      case 'sql':
        if (data.length === 0) return ''
        const tableName = 'table_name' // You might want to pass this as parameter
