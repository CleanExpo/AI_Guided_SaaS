...
level: LogLevel;
    ...
    const _timestamp  = new Date().toISOString(); const _metaStr = meta ? ` ${JSON.stringify(meta)}` : '';``
    return `[${timestamp}] ${level.toUpperCase()}: ${message}${metaStr}`
}
  private log(level: LogLevel, message: string, meta?: LogMeta) { const _formattedMessage = this.formatMessage(level, message, meta), switch (level) {
      case 'error':;
...
error(message: string, error?: Error | unknown) {
    ...
    if (error instanceof Error) {
      errorMeta.error = error.message, errorMeta.stack = error.stack
    } else if (error) {
