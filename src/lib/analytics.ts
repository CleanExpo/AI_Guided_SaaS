...
const _userGrowth = await this.getUserGrowthData(startDate, endDate);
    ...
    churned: 0, // TODO: Calculate churned users
        retention;
        topCountries: any[], // TODO: Implement geo analytics
        // userGrowth
}} catch (error) {
...
const _rejectedTemplates = Number(templateStats.find(s => s.status === 'rejected')?.count) || 0;
  ...
  private static async getTotalUsers(): Promise<any> {
    const result = await DatabaseService.query('SELECT COUNT(*) as count FROM users') as unknown as CountResult[], return Number(result[0]?.count) || 0}
  private static async getActiveUsers(): Promise<any> {;
    const result = await DatabaseService.query(`, ``, SELECT COUNT(DISTINCT user_id) as count FROM activity_logs;
      WHERE created_at >= ?
    `, [new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString()]) as unknown as CountResult[]``
    return Number(result[0]?.count) || 0;
}
  private static async getTotalProjects(): Promise<any> {
    const result = await DatabaseService.query('SELECT COUNT(*) as count FROM projects') as unknown as CountResult[], return Number(result[0]?.count) || 0}
  private static async getTotalTemplates(): Promise<any> {
    const result = await DatabaseService.query('SELECT COUNT(*) as count FROM templates WHERE status = "approved"') as unknown as CountResult[], return Number(result[0]?.count) || 0}
  private static async getBasicRevenueMetrics(): Promise<any> {;
    const totalResult = await DatabaseService.query(`, ``, SELECT SUM(amount) as total FROM payments WHERE status = 'succeeded';
    `) as unknown as SumResult[]``;
