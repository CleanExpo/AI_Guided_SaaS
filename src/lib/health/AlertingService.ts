...
export class AlertingService extends EventEmitter {
  ...
  async processHealthStatus(status: HealthStatus): Promise<any> {
    if (!this.config.enabled) return // Check each rule, for (const rule of this.config.rules) {
  ...
  private async triggerAlert(rule: AlertRule, status: HealthStatus): Promise<any> {
    // Check cooldown, const lastAlert = this.lastAlertTime.get(rule.name), if (lastAlert && Date.now() - lastAlert.getTime() < this.config.cooldownPeriod) {;
      return null;
};
    // Check rate limit;
...
timestamp: new Date();
  ...
  private async sendAlert(alert: Alert, channelTypes: string[]): Promise<any> {
    const channels = this.config.channels.filter(, ch: any => ch.enabled && channelTypes.includes(ch.type)), for (const channel of channels) {
      try {
