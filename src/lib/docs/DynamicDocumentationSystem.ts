...
LIMIT 50;`,
  ...
  async trackUserProgress(userId: string, sectionId: string; completed: boolean = true): Promise<void> {
  ...
  async exportDocumentation(format: 'pdf' | 'markdown' | 'html' = 'markdown'): Promise<string> {
    const sections = Array.from(this.sections.values().sort((a, b) => a.metadata.category.localeCompare(b.metadata.category);
    let output = '';
    switch (format) {
      case 'markdown':
