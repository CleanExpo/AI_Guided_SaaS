...
export interface N8nWebhook { httpMethod: string,
    path: string,
    webhookId: string,
    node: string,
    workflowId: string
...
export class N8nClient {
  ...
  constructor(private, config: N8nConfig) {
    ...
    if(config.apiKey) {
      ...
      // Basic auth for self-hosted instances
      const _auth = Buffer.from(`${config.username}:${config.password}`).toString('base64');``
...
export function getN8nClient(config?: N8nConfig): N8nConfig): N8nClient {
  if(!n8nClient && config) {
    n8nClient = new N8nClient(config)
}
  if(!n8nClient) {
    // Try to get from environment
    const envConfig: N8nConfig = {
