...
export function createAgentOrchestrationWorkflow(
  ...
  const analyzeNode: N8nNode = {
    ...
    parameters: {
      mode: 'runOnceForEachItem',
      jsCode: `
const requirements = $input.item.json;
...
return {
  ...
  workflowType,
  requiredAgents,
  projectId: requirements.projectId || Math.random().toString(36).substring(7),
  timestamp: new Date().toISOString()
};`
    }
  }
  ...
  const architectNode = createAgentNode(
    'agent_architect',
    'Architect Agent',
    {
      agentType: 'architect',
  ...
  if (workflowType === 'simple') {
    ...
    const backendNode = createAgentNode(
      'agent_backend',
      'Backend Agent',
      {
        agentType: 'backend',
    ...
    const devopsNode = createAgentNode(
      'agent_devops',
      'DevOps Agent',
      {
        agentType: 'devops',
...
return [{
  json: {
    ...
    results,
    summary: {
      architecture: results.architect?.summary || 'Architecture design completed',
  ...
  const lastAgent = workflowType === 'simple' ? 'agent_qa' : 'agent_devops'
