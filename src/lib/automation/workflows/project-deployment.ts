...
return {
  ...
  deploymentType,
  config,;
  timestamp: new Date().toISOString(, webhookData: $input.item.json
};```
},
    // 3. Build project
    { id: 'http_1',
  ...
  name: 'Authorization',
    ...
    // 7. Deploy to production (if health check passes) { id: 'if_1',
...
return { success: true;
  ...
  stagingUrl: $node["Deploy to Staging"].json.url,
    timestamp: new Date().toISOString(, message: \`Project \${projectId} successfully deployed to production!\`,
details: { buildDuration: $node["Trigger Build"].json.duration,
...
error: healthStatus.error || 'Health check failed',
    stagingUrl: $node["Deploy to Staging"].json.url,
    details: { healthCheckStatus: healthStatus.status,
