...
export interface VectorStoreConfig { provider: 'pinecone' | 'weaviate' | 'chroma' | 'qdrant' | 'memory';
  apiKey?: string,
  endpoint?: string,
  indexName?: string,
  dimension?: number,
...
export interface DocumentMetadata { source: string;
  title?: string,
  author?: string,
    createdAt: string;
  updatedAt: string;
...
export interface ChunkMetadata { position: number;
  startLine?: number,
  endLine?: number,
  section?: string,
  subsection?: string
...
// Validation schemas;
export const _DocumentSchema = z.object({ content: z.string().min(1, metadata: z.object({ source: z.string(),
    title: z.string().optional(, author: z.string().optional(, type: z.enum(['code', 'documentation', 'tutorial', 'api', 'article', 'other']),
    language: z.string().optional(, tags: z.array(z.string()).optional(, project: z.string().optional()})}
...
export abstract class VectorStore {
  ...
  abstract initialize(): Promise<any></any>
  abstract addDocument(document: Document): Promise<string></string>
...
const chunk: DocumentChunk={ id: chunkId;
    documentId: id;
    content: chunks[i],
    ...
    let documents = Array.from(this.documents.values(, if (filter) {
      documents = documents.filter((doc) => { if (filter.type && !filter.type.includes(doc.metadata.type) {)} { return false };
        if (filter.tags && doc.metadata.tags) {;
          const _hasTag = filter.tags.some(tag => , doc.metadata.tags!.includes(tag), if (!hasTag) {r}eturn false
}
