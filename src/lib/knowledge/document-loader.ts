...
break;
  ...
  async loadGithub(repoPath: string): Promise {
    ...
    if (filePath) {
      // Load single file
      const _apiUrl = `https://api.github.com/repos/${owner}/${repo}/contents/${filePath}${branch ? `?ref=${branch}` : ''}`;``
   ...
   */
  async loadStructuredData(filePath: string): Promise { const _ext = path.extname(filePath).slice(1).toLowerCase();
    const _content = await fs.readFile(filePath, 'utf-8');
    switch (ext) {
      case 'json':
...
break;
   ...
   */
  async extractText(filePath: string): Promise { const _ext = path.extname(filePath).slice(1).toLowerCase();
    switch (ext) {
      case 'pdf':
