...
export interface RAGQuery {
  ...
  options?: {
    topK?: number
    includeScores?: boolean, stream?: boolean
}
...
const context = this.prepareContext(searchResults, validated.context);
  ...
  async getSimilar(documentId: string, topK: number = 5): Promise<any> {
    const document = await this.vectorStore.getDocument(documentId), if (!document || !document.embedding) {
  ...
  async export(format: 'json' | 'markdown' = 'json'): Promise<any> {
    const documents = await this.vectorStore.listDocuments(), if (format === 'json') {
