...
export class RAGEngine {
  ...
  async ingestCodebase(path: string, options?: {;
    ...
    try {
      const _files = await this.documentLoader.loadCodebase(path, options);
  ...
  async updateDocument(id: string, content?: string, metadata?: Partial<Document['metadata']>): Promise {
    ...
    if (metadata) { const existing = await this.vectorStore.getDocument(id);
      if (existing) {
        update.metadata = {
          ...existing.metadata,
          ...metadata,
...
Answer:`
  ...
  private async *streamResponse(
    ...
    const _words = 'This is a streaming response based on the context provided.'.split(', ');
