...
export class InteractiveTutorialSystem extends EventEmitter {
  ...
  private async showStep(tutorial: Tutorial, progress: TutorialProgress, stepIndex: number) {
  ...
  private customValidation(target: string, expected): boolean {
  ...
  private async completeTutorial(tutorial: Tutorial, progress: TutorialProgress) {
  ...
  private async saveProgress(progress: TutorialProgress) {
    // Save to database
    try {
