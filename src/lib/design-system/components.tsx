...
export function UnifiedAlert({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0 y: -10 }}
      className={cn(`
        'flex items-start p-4 rounded-lg border' config.bg className
      )}
    >
...
}: UnifiedBadgeProps), size  = 'sm', icon, dot  = false, children, className
}: UnifiedBadgeProps) {
  const variants = {
      ...
      {variant === 'linear' ? (</div>
        <div className="flex items-center justify-between">
          {steps.map((step, index) => {
            const _isActive = index === currentStep;</div>
            const _isCompleted = index < currentStep
            return (
    <React.Fragment key={step.id}>
                <div className="flex flex-col items-center flex-1"></div>
                  <motion.div
                    className={cn("w-10 h-10 rounded-full flex items-center justify-center font-medium transition-colors", isCompleted ? 'bg-primary-600 text-white' :
