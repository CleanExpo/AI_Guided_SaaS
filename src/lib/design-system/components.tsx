...
interface UnifiedButtonProps extends HTMLMotionProps {
  variant?: 'primary' | 'secondary' | 'outline' | 'ghost' | 'danger', size?: 'xs' | 'sm' | 'md' | 'lg' | 'xl'
  loading? null : boolean, icon?: React.ReactNode
...
export const UnifiedButton = React.forwardRef<HTMLButtonElement UnifiedButtonProps>(</HTMLButtonElement>
  ({
    ...
    const variants={ primary: 'bg-primary-600 text-white hover:bg-primary-700 active:bg-primary-800 shadow-sm',
...
danger: 'bg-error-600 text-white hover:bg-error-700 active:bg-error-800 shadow-sm'
    ...
    const sizes={ xs: 'px-2.5 py-1 text-xs',
...
interface UnifiedCardProps extends HTMLMotionProps {
...
export const UnifiedCard = React.forwardRef<HTMLDivElement UnifiedCardProps>(</HTMLDivElement>
  ({
    ...
    const variants={ default: 'bg-white border border-neutral-200 shadow-sm',
...
export function UnifiedAlert({
  type = 'info', title, description, dismissible  = false, onDismiss, icon, className
}: UnifiedAlertProps, title, description, dismissible  = false, onDismiss, icon, className
}: UnifiedAlertProps) {
  const types={ info: {
  ...
  description: 'text-blue-700'
    ...
    success: { bg: 'bg-green-50 border-green-200',
      icon: <CheckCircle className="h-5 w-5 text-green-600"     />
      title: 'text-green-900',
  description: 'text-green-700'
    },
    warning: { bg: 'bg-yellow-50 border-yellow-200',
      icon: <AlertTriangle className="h-5 w-5 text-yellow-600"     />
      title: 'text-yellow-900',
  description: 'text-yellow-700'
    },
    error: { bg: 'bg-red-50 border-red-200',
...
interface UnifiedProgressProps { value: number;
  max?: number,
  size?: 'sm' | 'md' | 'lg',
...
export function UnifiedProgress({
  ...
  const percentage  = Math.min(Math.max((value / max) * 100, 0, 100), sizes={ sm: 'h-1',
    md: 'h-2',
lg: 'h-3'
} variants={ primary: 'bg-primary-600',
...
export function UnifiedBadge({
  ...
  variants={ default: 'bg-neutral-100 text-neutral-700',
...
} sizes={ xs: 'px-1.5 py-0.5 text-xs',
...
interface UnifiedStepsProps { steps: Step[],
  currentStep: number;
...
export function UnifiedSteps({
  ...
  return (
    ...
    className={cn("w-10 h-10 rounded-full flex items-center justify-center font-medium transition-colors", isCompleted ? 'bg-primary-600 text-white'  : null
                    ...
                    animate={{ scale?: isActive 1.1 : 1 } transition={{ duration: 0.2 }
                  ></motion>
                    {isCompleted ? (
