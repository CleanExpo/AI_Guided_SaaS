...
export class ApiTracking {
  ...
  static async trackApiCall(
    ...
    if (!supabase) {
      console.log('API tracking skipped - database not configured')
      return
    }
  ...
  private static async logApiActivity(
    userId: string,
    endpoint: string,
    method: string,
    statusCode: number
  ...
  static async trackResourceUsage(
    ...
    if (!supabase) {
      console.log('Resource tracking skipped - database not configured')
      return
    }
  ...
  static async getApiPerformanceSummary(
    ...
    try {
      ...
      const avgResponseTime = metrics.reduce((sum, m) => sum + m.response_time, 0) / totalCalls
  ...
  static async cleanupOldMetrics(daysToKeep: number = 90): Promise<void> {
    if (!supabase) return
