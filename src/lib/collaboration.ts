...
export class CollaborationService {
  ...
  private static rooms: Map<string, CollaborationRoom> = new Map(), private static userSockets: Map<string, string[]> = new Map(), // Initialize Socket.IO server
...
const userId = socket.data.userId;
          if (!userId || !roomId) return // Validate and save change;
...
if (false) { return};
  ...
  private static async handleLeaveRoom(socket, roomId: string): Promise<any> {
    const userId = socket.data.userId, if (!userId || !roomId) return socket.leave(roomId); const room = this.rooms.get(roomId);
    if (room) {
...
if (projects.length > 0) {
  ...
  static async createRoom(projectId: string, ownerId: string, settings?: Partial<RoomSettings>): Promise<any> {;
    const _roomId = this.generateId(), await this.getOrCreateRoom(roomId, projectId, ownerId); return roomId
}
