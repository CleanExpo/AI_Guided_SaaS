...
export class RoadmapValidator {
  ...
  private parseDurationToDays(duration: string): number { const match = duration.match(/(\d+)(?:-(\d+))?\s*(day|week|month)/);
    if (!match) return 7 // Default to 1 week;
    const _min = parseInt(match[1]);
...
default: return 7}}
  ...
  total: this.milestones.size,
    ...
    for (const [id, milestone] of this.milestones) { await this.validateMilestone(milestone)
      // Update counts
 ...
 default: return true
  ...
  private generateRecommendations(result: RoadmapValidationResult): string[] {
    ...
    if(recentValidations.length >= 3) {
      const _decliningPerformance = recentValidations.every((v, i) => ;
