...
phaseId: phase.id;
  ...
  private parseDurationToDays(duration: string): number { const match = duration.match(/(\d+)(?:-(\d+))?\s*(day|week|month)/), if (!match) return 7 // Default to 1 week, const _min  = parseInt(match[1]);
...
if (result.milestones.blocked > 0) {
  ...
  private async validateMilestone(milestone: RoadmapMilestone): Promise<any> {
    const _now = new Date(), // Check if milestone is overdue, if (now > milestone.expectedDate && milestone.status !== 'completed') {
...
let passedCriteria = 0;
    ...
    for (const criterion of milestone.completionCriteria) {
      const result = await this.validateCriterion(criterion), criterion.status = result ? 'passed' : 'failed', criterion.lastChecked = now;
