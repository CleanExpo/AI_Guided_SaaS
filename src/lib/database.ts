...
export interface ProjectFiles { [path: string]: { content: string
}
  type: 'file' | 'directory';
    size?: number
}
...
export interface UsageRecord { id: string;
  user_id: string;
  resource_type: string;
  quantity: number;
...
if (error) {
  ...
  status: 'completed',
    config: {},
    created_at: new Date().toISOString(, updated_at: new Date().toISOString()} { id: 'demo-project-2',
  user_id: userId;
...
if (error) {
  ...
  stripe_customer_id: 'mock-stripe-customer',
        tier: 'free',
  status: 'active',
        current_period_start: new Date().toISOString(, current_period_end: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(, cancel_at_period_end: false
}
  created_at: new Date().toISOString(, updated_at: new Date().toISOString()}
    try {
      const { data, error    }: any = await supabase!;
...
if (error) {
  ...
  static async getUserUsage(userId: string, month? null : Date): Promise { if (!this.checkDatabase() {)} {
      return { projects: 2 }
  aiGenerations: 15;
    storage: '125MB'
  }
...
const projectCount = await this.getUserProjects(userId);
  ...
  private static hashUserId(userId: string): number {
    let hash = 0, for (let i = 0, i < userId.length; i++) {
      const _char = userId.charCodeAt(i); hash = ((hash << 5) - hash) + char;
