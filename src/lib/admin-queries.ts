...
export class AdminQueries {
  ...
  static async getAdminStats(): Promise<AdminStats> {
    ...
    try {
      ...
      const systemHealth = 'healthy' // TODO: Implement real health checks
  ...
  static async getAnalytics(range: string): Promise<AnalyticsData> {
    if (!supabase) {
      throw new Error('Database not configured')
    }
  ...
  private static formatActivityMessage(activity: ActivityLog): string {
  ...
  private static async getUserMetrics(startDate: Date, days: number) {
    ...
    const retentionRate = totalUsers ? (activeUsersByDate[activeUsersByDate.length - 1].count / totalUsers) * 100 : 0
  ...
  private static async getProjectMetrics(startDate: Date, days: number) {
    ...
    const successRate = totalProjects ? Math.round((completedProjects! / totalProjects) * 100) : 0
  ...
  private static async getApiMetrics(startDate: Date, days: number) {
    ...
    const topEndpoints = [
      { endpoint: '/api/chat', calls: Math.floor(totalCalls * 0.4), avgTime: 145 },
      { endpoint: '/api/generate', calls: Math.floor(totalCalls * 0.25), avgTime: 2340 },
      { endpoint: '/api/projects', calls: Math.floor(totalCalls * 0.15), avgTime: 89 },
      { endpoint: '/api/auth', calls: Math.floor(totalCalls * 0.12), avgTime: 56 },
      { endpoint: '/api/export', calls: Math.floor(totalCalls * 0.08), avgTime: 1240 }
    ]
  ...
  private static async getRevenueMetrics() {
    ...
    const churnRate = totalSubs ? Math.round((canceledSubscriptions! / totalSubs) * 100 * 10) / 10 : 0
