...
export class AdminQueries {
  ...
  static async getAdminStats(): Promise {
    ...
    try {
      ...
      const { data: activeSessions   }: any = await supabase;
        .from('sessions')
        .select('user_id')
        .gte('expires', thirtyDaysAgo.toISOString());
      const _activeUsers = new Set(activeSessions?.map((s) => s.user_id) || []);
        .size;
      // Get new users today
      const todayStart = new Date();
      todayStart.setHours(0, 0, 0, 0);
      const { count: newUsersToday   }: any = await supabase;
        .from('users')
        .select('*', { count: 'exact', head: true })
      ...
      const { count: newUsersThisWeek   }: any = await supabase;
        .from('users')
        .select('*', { count: 'exact', head: true })
        .gte('created_at', weekStart.toISOString());
      // Get total projects
      const { count: totalProjects   }: any = await supabase;
        .from('projects')
        .select('*', { count: 'exact', head: true });
      // Get active projects (updated within last 7 days)
      const { count: activeProjects   }: any = await supabase;
        .from('projects')
        .select('*', { count: 'exact', head: true })
        .gte('updated_at', weekStart.toISOString());
      // Get API calls (from activity logs)
      const { count: apiCallsToday   }: any = await supabase;
        .from('activity_logs')
        .select('*', { count: 'exact', head: true })
        .eq('action', 'api_call')
        .gte('created_at', todayStart.toISOString());
      const { count: apiCallsThisWeek   }: any = await supabase;
        .from('activity_logs')
        .select('*', { count: 'exact', head: true })
        .eq('action', 'api_call')
        .gte('created_at', weekStart.toISOString());
      ...
      const { count: apiCallsThisMonth   }: any = await supabase;
        .from('activity_logs')
        .select('*', { count: 'exact', head: true })
        .eq('action', 'api_call')
        .gte('created_at', monthStart.toISOString());
      // Get recent activity
      const { data: recentActivityData   }: any = await supabase;
        .from('activity_logs')
        .select('*')
        .order('created_at', { ascending: false })
        .limit(10);
      const _recentActivity = (recentActivityData || []).map((activity) => ({
        type: activity.action,
    message: this.formatActivityMessage(activity),
    timestamp: activity.created_at
      }});
      // Calculate system health (simplified for now)
      const _systemHealth = 'healthy'; // TODO: Implement real health checks
      const _uptime = '99.9%'; // TODO: Calculate from monitoring data
      const _cpuUsage = '45%'; // TODO: Get from system metrics
      const _memoryUsage = '62%'; // TODO: Get from system metrics
      return {;
  ...
  static async getAnalytics(range: string): Promise {
    if(!supabase) {
      throw new Error('Database not configured');
}
    const _days =;
    ...
    try {
      // Get user metrics
      const userMetrics = await this.getUserMetrics(startDate, days);
      // Get project metrics
      ...
      // Get platform health
      const _platformHealth = await this.getPlatformHealth();
      return {;
        overview: {
  ...
  static async getUsers(params: {
    ...
    try {
      ...
      const _enrichedUsers = await Promise.all(;
        (users || []).map(async user: (any) => {
          // Get user's project count
          const { count: projectsCount   }: any = await supabase!;
            .from('projects')
            .select('*', { count: 'exact', head: true })
            .eq('user_id', user.id);
          ...
          const _lastSession = await supabase!;
            .from('sessions')
            .select('expires')
            .eq('user_id', user.id)
  ...
  private static async getUserMetrics(startDate: Date, days: number): Promise {
    // Generate daily user metrics
    const dates: any[] = [];
    const newUsersByDate: any[] = [];
    ...
    for(let i = 0; i < days; i++) {
      ...
      const _activeCount = new Set(sessions?.map((s) => s.user_id) || []).size;
      dates.push(dateStr);
      newUsersByDate.push({
    ...
    const { count: totalUsers   }: any = await supabase!;
      .from('users')
      .select('*', { count: 'exact', head: true });
    const _retentionRate = totalUsers;
  ...
  private static async getProjectMetrics(startDate: Date, days: number): Promise {
    // Get total projects
    const { count: totalProjects   }: any = await supabase!;
      .from('projects')
      .select('*', { count: 'exact', head: true });
    // Generate daily project creation metrics
    const projectsByDate: any[] = [];
    for(let i = 0; i < days; i++) {
      const date = new Date(startDate);
      date.setDate(date.getDate() + i);
      const nextDate = new Date(date);
    ...
    const _projectTypes = Object.entries(typeCounts).map(([type, count]) => ({
    ...
    const { count: completedProjects   }: any = await supabase!;
      ...
      .select('*', { count: 'exact', head: true })
      .eq('status', 'completed');
    const _successRate = totalProjects;
  ...
  private static async getApiMetrics(startDate: Date, days: number): Promise {
    // Generate daily API call metrics
    const callsByDate: any[] = [];
    const latencyByDate: any[] = [];
    let totalCalls = 0;
    for(let i = 0; i < days; i++) {
      const date = new Date(startDate);
      date.setDate(date.getDate() + i);
  ...
  private static async getRevenueMetrics(): Promise {
    ...
    const { count: activeSubscriptions   }: any = await supabase!;
      .from('subscriptions')
      .select('*', { count: 'exact', head: true })
      .eq('status', 'active');
    ...
    const _totalSubs = (activeSubscriptions || 0) + (canceledSubscriptions || 0);
    const _churnRate = totalSubs;
      ? Math.round((canceledSubscriptions! / totalSubs) * 100 * 10) / 10
      : 0;
    return {;
      totalRevenue: totalRevenue / 100;
  // Convert from cents, activeSubscriptions: activeSubscriptions || 0,
      churnRate;
}
  private static async getPlatformHealth(): Promise { // These would come from real monitoring systems in production
    return {;
      uptime: 99.95,
    avgResponseTime: 178,
    errorRate: 0.12,
