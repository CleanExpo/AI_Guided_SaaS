...
class ConfigurationManager {
  ...
  private parsePropertiesFile(content: string): Record {
    ...
    for(const line of lines) {
      ...
      if(equalIndex > 0) {
        const _key = trimmedLine.substring(0, equalIndex).trim();
...
break;
   ...
   */
  async getPrimaryModelForTask(task: 'code_generation' | 'documentation' | 'analysis' | 'review'): Promise { const aiProvider = await this.getAIProviderConfig();
