...
const MASTER_ADMIN = {
  ...
  role: 'super_admin' as const status: 'active' as const password: process.env.ADMIN_PASSWORD || '',
  permissions: [
    'manage_users',
    'moderate_content',
    'view_analytics',
...
export class AdminAuthService { private static instance: AdminAuthService;
  ...
  generateAdminToken(admin: AdminUser) {
    try {
  ...
  verifyAdminToken(token: string): AdminSession | null {
    try {
      const _decoded = jwt.verify(token, this.jwtSecret, {
        issuer: 'ai-guided-saas-admin',
        audience: 'admin-panel'
      }) as AdminSession;
      return decoded;
  ...
  async verifyAdminSession(request: NextRequest): Promise {
    try {
      const _token = this.extractAdminToken(request);
      if(!token) {
        return null;
}
  ...
  // Log admin activity
  async logAdminActivity(session: AdminSession, action: string, target: string, details: Record<string, unknown> = {}, ipAddress: string = 'unknown'): Promise {
    try {
      const activity = {
  ...
  async getAdminById(adminId: string): Promise {
    try {
      // Check master admin
  ...
  async validateAndRefreshSession(request: NextRequest): Promise {
    try {
      ...
      const _expiresIn = session.exp - Math.floor(Date.now() / 1000);
      if(expiresIn < 3600) { // Less than 1 hour
        const admin = await this.getAdminById(session.adminId);
        if (admin) {
          const _newToken = this.generateAdminToken(admin);
...
export const _verifyAdminToken = (token: string) => adminAuthService.verifyAdminToken(token);
...
export async function requireAdminAuth(
  ...
  try {
    ...
    } catch (error) { console.error('Error in admin auth middleware:', error);
    return {;
      authorized: false,
...
export function createAdminResponse(;
  data: unknown, status: number = 200, newToken?: string): unknown, status: number = 200, newToken?: string): Response {
