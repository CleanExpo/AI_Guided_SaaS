...
export class MCPOrchestrator {
  ...
  async registerServer(
    server: Omit<MCPServer, 'status' | 'tools'>
  ): Promise<void> {
    try {
      // Connect to server
  ...
  async callTool(call: MCPToolCall): Promise<MCPToolResult> {
    ...
    try {
      ...
      if (!tool) {
        throw new Error(
          `Tool ${validated.tool} not found on server ${validated.server}`
        );
      }
   ...
   * Read a resource
   */
  async readResource(serverId: string,
  ...
  private async connectToServer(serverId: string,
