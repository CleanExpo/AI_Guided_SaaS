...
for (const missing of epcResult.missing) {
      const action = await this.generateMissingVarAction(missing), if (action) plan.actions.push(action)}
    for (const invalid of epcResult.invalid) {
      const action = await this.generateInvalidVarAction(invalid), if (action) plan.actions.push(action)}
    for (const outdated of epcResult.outdated) {
      const action = await this.generateOutdatedVarAction(outdated), if (action) plan.actions.push(action)};
...
h: any =>
  ...
  async getAISuggestions(issues: string[]): Promise<any> {
    const _prompt = `As a DevOps expert, analyze these environment configuration issues and suggest, fixes: ``, ${issues.join('\n')}
