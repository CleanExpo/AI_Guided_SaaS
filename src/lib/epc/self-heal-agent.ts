...
interface HealingPlan { issues: string[],
  actions: HealingAction[],
  estimatedTime: string;
  confidence: number
};
export class SelfHealingAgent {
  private epcEngine: EPCEngine, private envManager: EnvManager, private aiService: AIService;
...
for (const missing of epcResult.missing) {
      const action = await this.generateMissingVarAction(missing, if (action) {p}lan.actions.push(action)}
    for (const invalid of epcResult.invalid) {
      const action = await this.generateInvalidVarAction(invalid, if (action) {p}lan.actions.push(action)}
    for (const outdated of epcResult.outdated) {
      const action = await this.generateOutdatedVarAction(outdated, if (action) {p}lan.actions.push(action)}; // Calculate confidence based on automated actions; const _automatedCount = plan.actions.filter((a) => a.automated).length;
    plan.confidence =
...
{{ success: true;
   ...
   */
  private async applyAction(action: HealingAction): Promise { switch (action.type) {
      case 'sync_config':
      await this.envManager.sync(, break, case 'update_env':
...
h: any =>
   ...
   */
  private estimateHealingTime(actions: HealingAction[]) {
    const _automated = actions.filter((a) => a.automated).length; const manual = actions.filter((a) => !a.automated).length, if (manual === 0) {;
      return 'Less than 1 minute'
} else if (manual <= 3) {
      return '2-5 minutes'} else {
      return '5-10 minutes'}
}
  /**
