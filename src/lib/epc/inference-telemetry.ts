...
interface TelemetryEntry { timestamp: string;
  ...
  preflightCheck: { passed: boolean;
  score: number;
  issues: string[]
}
    environmentSnapshot: { nodeEnv: string;
    activeServices: string[],
    memoryUsage: NodeJS.MemoryUsage,
    uptime: number
  },
    inference: { started: boolean;
    completed: boolean;
    duration?: number, tokensUsed?: number, cost?: number;
    error?: string
  };
  agentContext? null : { agentId: string;
    agentType: string;
    taskType: string
  },
...
export class InferenceTelemetry {
  private telemetryDir: string, private currentLog: string, private buffer: TelemetryEntry[] = [];
...
{ await this.epcEngine.performPreflightCheck(); const entry: TelemetryEntry={ timestamp: new Date().toISOString();
      requestId,
      inferenceType,
    preflightCheck: { passed: preflightResult.env_check === 'pass',
  ...
  ...preflightResult.outdated.map((v) => `outdated: ${v}`, ``
    ...
    return { allowed: preflightResult.action !== 'block_inference',
...
logInferenceStart(requestId: string) {
    const entry = this.buffer.find(e => e.requestId === requestId, if (entry) {
      entry.inference.started = true, entry.inference.startTime = Date.now()}
  /**
...
const _eventLog = path.join();
   ...
   */
  private flushBuffer() {
    if (this.buffer.length === 0) {r}eturn null, try {
      let existingData: TelemetryEntry[] = [], if (fs.existsSync(this.currentLog) {)} {;
...
const issueCount = new Map<string number>();</string>
      ...
      for (const file of files) {
        ...
        for (const entry of data) {
          ...
          stats.totalInferences++; if (entry.outcome === 'blocked') {s}tats.blocked++;
          else if (entry.outcome === 'failed') {s}tats.failed++;
          else if (entry.outcome === 'success') {s}tats.successful++;
          if (entry.inference.duration) {
            totalDuration += entry.inference.duration, durationCount++
}
...
cleanupOldFiles(daysToKeep: number = 30) {
    const cutoffDate = new Date(), cutoffDate.setDate(cutoffDate.getDate() - daysToKeep, try {;
