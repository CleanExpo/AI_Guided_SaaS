...
export class InferenceTelemetry {
  ...
  async logPreInference(requestId: string, inferenceType: TelemetryEntry['inferenceType'], agentContext?: TelemetryEntry['agentContext']): Promise<any> {
    const preflightResult = await this.epcEngine.performPreflightCheck(); const entry: TelemetryEntry = {
...
logInferenceComplete(requestId: string;
  ...
  private getActiveServices(): string[] {
    const services: string[] = [], if (process.env.OPENAI_API_KEY) services.push('openai'); if (process.env.CLAUDE_API_KEY) services.push('anthropic');
...
const _eventLog = path.join(
   ...
   */
  private flushBuffer() {
    if (this.buffer.length === 0) return null, try {
...
cleanupOldFiles(daysToKeep: number = 30) {
    const cutoffDate = new Date(), cutoffDate.setDate(cutoffDate.getDate() - daysToKeep), try {;
      ...
      for (const file of files) {
        ...
        if (match) { const _fileDate = new Date(match[1]), if (fileDate < cutoffDate) {
