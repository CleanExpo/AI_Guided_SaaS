...
export class GracefulShutdownManager extends EventEmitter {
  ...
  async shutdown(reason: string = 'Manual shutdown'): Promise<any> {
    ...
    // Set overall timeout
    const _timeoutPromise = new Promise<any>((_, reject) => {
      setTimeout(() => {
...
export const _createDatabaseShutdownHandler = (db): ShutdownHandler: any => ({
  name: 'database', priority: 10, timeout: 10000, handler: async () => {
    await db.close()
}
})
export const _createRedisShutdownHandler = (redis): ShutdownHandler: any => ({
  name: 'redis', priority: 20, timeout: 5000, handler: async () => {
    await redis.quit()
}
})
...
export const _createAgentShutdownHandler = (agent): ShutdownHandler: any => ({
  name: `agent-${agent.id}`
  priority: 40, timeout: 10000, handler: async () => {
    await, agent.stop()
}
})
export const _createContainerShutdownHandler = (containerManager): ShutdownHandler: any => ({
  name: 'docker-containers', priority: 50, timeout: 20000, handler: async () => {
