...
export class AgentSystem extends EventEmitter {
  ...
  public static getInstance(config?: AgentSystemConfig): AgentSystem {
  ...
  private initializeMCP(): void {
    ...
    this.mcpBridge = new AgentMCPBridge(this.mcpIntegration, {
      enabledServers: this.config.mcp?.servers || ['context7', 'sequential-thinking', 'serena'],
      autoConnect: true,
      cacheResponses: true
    });
  ...
  private initializePulsedExecution(): void {
    this.pulsedExecutor = new PulsedExecutor({
      minInterval: 1000,
      maxInterval: 3000,
  ...
  private handleCriticalResourceAlert(alert: any): void {
    if (alert.type === 'cpu' && this.pulsedExecutor) {
      ...
      this.pulsedExecutor.updateConfig({
        minInterval: 2000,
        maxInterval: 5000
      });
    }
  ...
  public getSystemMetrics(): any {
    return {
      ...
      resources: this.resourceMonitor?.getLatestSnapshot(),
      containers: this.containerOrchestrator?.getStatus(),
      pulsedExecution: this.pulsedExecutor?.getResourceMetrics()
    };
  }
