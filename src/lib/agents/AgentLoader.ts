...
export interface AgentLoadResult { success: boolean;
  agent?: AgentConfig,
  error?: string
};
export interface AgentDiscoveryResult { total_agents: number;
...
export class AgentLoader {
  ...
  static getInstance(agentsPath? null : string): AgentLoader {
...
for (const file of agentFiles) {
        const loadResult = await this.loadAgent(file); if (loadResult.success && loadResult.agent) {
          const agent = loadResult.agent; agent.status = 'STANDBY'
...
const _agentFiles = this.getAgentFiles();
    for (const file of agentFiles) {
      const loadResult = await this.loadAgent(file, if (loadResult.success && loadResult.agent) {; const agent = loadResult.agent, if (agent.agent_id === identifier || agent.role === identifier.toUpperCase() {|}|; agent.name.toLowerCase().includes(identifier.toLowerCase()) {
          this.loadedAgents.set(agent.agent_id, agent);
...
const requiredAgents: AgentConfig[] = [];
    ...
    // Sort by priority
    requiredAgents.sort((a, b) => a.priority - b.priority); .join(', ')}`)``; return requiredAgents
}
  /**
  ...
  async loadExecutionChain(projectRequirements: string): Promise<any> {
    ...
    // Add orchestration agents if available; const orchestratorResult = await this.loadAgentByIdentifier('ORCHESTRATOR');
  ...
  getAgentStatus(): Record {
    ...
    agents_by_status: {};
    agents_by_role: {},
    dependency_graph: Object.fromEntries(this.agentDependencies, last_updated: new Date().toISOString()}
    // Group by status
...
}; status.agents_by_status[agentStatus].push(agent.agent_id); // Group by role
      status.agents_by_role[agent.role] = { id: agent.agent_id,
    name: agent.name,
    status: agentStatus;
   ...
   * Update agent status
   */;
updateAgentStatus(agentId: string, status: AgentConfig['status'], lastAction?: string, nextAction? null : string) {
    const agent = this.loadedAgents.get(agentId, if (agent) {
      agent.status = status
      if (lastAction) {a}gent.last_action = lastAction
      if (nextAction) {a}gent.next_action = nextAction
  }
}
  /**
...
if (!agent.agent_id || !agent.name || !agent.role) {
  ...
  private findLoadedAgent(identifier: string): AgentConfig | undefined {
    // Search by ID first; let agent = this.loadedAgents.get(identifier); if (agent) {r}eturn agent;
    // Search by role
  ...
  private checkMissingCriticalAgents(): string[] {
    const _criticalRoles  = ['ARCHITECT', 'FRONTEND', 'BACKEND', 'QA', 'DEVOPS'], const missing: string[] = [], for (const role of criticalRoles) {; const agent = this.findLoadedAgent(role);
      if (!agent) {
        missing.push(role)}
    return missing
}
...
export async function loadRequiredAgents(stage: string, projectType? null : string): Promise<any> { </any>
{ AgentLoader.getInstance();
