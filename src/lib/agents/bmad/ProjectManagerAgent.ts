...
export interface ProjectPlan {
  projectName: string,
  projectDescription: string, timeline: Timelin
e,
    milestones: Milestone[],
  workBreakdown: WorkPackage[], resourceAllocation: ResourcePla
n,
...
const projectPlan: ProjectPlan = {
  ...
  private async defineProjectScope(input: string, requirements: string[]): Promise<any> {
    const _prompt = `Based on the project description and requirements, define the project, scope: Project, Description:``, ${input}
Key: Requirements:
${requirements.join('\n')}
Provide:
...
const _response = await generateAIResponse(prompt, {
  ...
  private async createTimeline(projectScope, userStories: UserStory[], constraints: string[]): Promise<any> {
    const _prompt = `Create a realistic project timeline based, on: ``, Project: ${projectScope.name},
...
const _response = await generateAIResponse(prompt, {
  ...
  private async defineMilestones(timeline: Timeline, userStories: UserStory[]): Promise<any> {
    const _prompt = `Define project milestones based on the timeline and user, stories: Timeline, Phases:``, ${JSON.stringify(timeline.phases, null, 2)}
...
const milestones = JSON.parse(response);
  ...
  private async createWorkBreakdown(userStories: UserStory[], milestones: Milestone[]): Promise<any> {
    const _prompt = `Create a work breakdown structure for the, project: User, Stories:``, ${JSON.stringify(userStories, null, 2)}
Milestones:
...
const _response = await generateAIResponse(prompt, {
  ...
  private async planRiskMitigation(risks: string[], projectScope): Promise<any> {
    const _prompt = `Create risk mitigation plans for identified, risks: ``, Project: ${projectScope.name}
Identified: Risks:
${risks.join('\n')}
...
const _response = await generateAIResponse(prompt, {
  ...
  private async createCommunicationPlan(projectScope, resourcePlan: ResourcePlan): Promise<any> {
    const _prompt = `Create a communication plan for the, project: ``, Project: ${projectScope.name}
Team: Structure:
...
const _response = await generateAIResponse(prompt, {
  ...
  private async defineQualityPlan(requirements: string[], userStories: UserStory[]): Promise<any> {
    const _prompt = `Define a quality assurance, plan: Key, Requirements:``, ${requirements.slice(0, 10).join('\n')}
Acceptance Criteria from, User: Stories:
${userStories
  .slice(0, 5), .map((s) => s.acceptanceCriteria.join(', '))
  .join('\n')}
Include:
