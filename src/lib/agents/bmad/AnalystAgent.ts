...
export class AnalystAgent extends Agent {
  ...
  private async extractRequirements(input: string): Promise<string> {
    const prompt = `As a expert requirements analyst, extract all explicit and implicit requirements from the following project description. Include functional features, quality attributes, constraints, and any, assumptions: Project; Description:`
${input}
...
Provide specific, measurable, achievable, relevant, and time-bound (SMART) criteria.`;`
  ...
  private async analyzeTechnicalAspects(
    ...
    const prompt = `Analyze the technical aspects and considerations for this, project: Project; Description:`
${input}
Requirements:
${JSON.stringify(requirements, null, 2)}
Identify:
