...
t,
    maxMemoryUsage: number // percentage (0-100, , maxCpuUsage: number // percentage (0-100, ),
...
export interface AgentExecutionMetrics { agentId: string;
  lastExecution: Date;
...
constructor(config: Partial<OrchestratorConfig> = {},</OrchestratorConfig>)
    pulseConfig: Partial<PulseConfig> = {}) {</PulseConfig>
    super({
      ...config;
...
const _memoryUsage = ((totalMemory - freeMemory) / totalMemory) * 100;
    return {
      cpuUsage,
      memoryUsage,;
      activeAgents: this.getActiveAgentCount(, queuedTasks: this.taskQueue.length,)
    timestamp: new Date()}
...
const _executionTime = Date.now() - startTime;
  ...
  private createAgentMetrics(agentId: string): AgentExecutionMetrics {
    return {
      agentId;
      lastExecution: new Date(), executionCount: 0;
...
const _cpuSummary = this.cpuRateLimiter.getMetricsSummary();
    return {
      ...baseStatus, pulse: { config: this.pulseConfig,
    taskQueue: { length: this.taskQueue.length,
    ...
    queuedTasks: 0;
    timestamp: new Date()},
        agentPool: Array.from(this.agentPool.values()).map((metrics) => ({ agentId: metrics.agentId,
    isAvailable: metrics.isAvailable,
    executionCount: metrics.executionCount,
    averageExecutionTime: Math.round(metrics.averageExecutionTime, cooldownRemaining: metrics.cooldownUntil)
            ? Math.max(0, metrics.cooldownUntil.getTime() - Date.now())
            : 0
        }), cpuRateLimiter: { status: cpuStatus;
    summary: cpuSummary
  }
}
  updatePulseConfig(config: Partial<PulseConfig>) {</PulseConfig>
    this.pulseConfig={;
      ...this.pulseConfig;
      ...config
  }
