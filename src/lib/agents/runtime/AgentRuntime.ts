...
Format as JSON,
   ...
   */
  private async getOrCreateAgent(agentType: string): Promise<Agent> {
   ...
   */
  private updateSharedMemory(context: AgentContext): void {
  ...
  private updateMetrics(result: TaskResult): void {
    ...
    const totalDuration = Array.from(this.completedTasks.values())
