...
export class ResourceMonitor extends EventEmitter {
  ...
  private createSnapshot(): ResourceSnapshot {
    ...
    return {
      ...
      process: {
        cpu: this.calculateProcessCpu(processCpu),
        memory: processMemory.heapUsed + processMemory.external,
   ...
   */
  public getAverageResources(periodMs: number): {
    cpu: number,
    memory: number,
    loadAverage: number;
  } | null {
    ...
    const totals = relevantSnapshots.reduce((acc, snapshot) => ({
      cpu: acc.cpu + snapshot.cpu.usage,
    ...
    return {
      cpu: totals.cpu / relevantSnapshots.length,
