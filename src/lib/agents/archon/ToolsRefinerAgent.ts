...
export class ToolsRefinerAgent extends Agent {
  ...
  constructor() {
    super({
      ...
      description: 'Expert in tool analysis, selection, and optimization. Refines tool sets to maximize agent capabilities while minimizing complexity and cost.',
      capabilities: [
        'Tool analysis',
        'Tool selection',
  ...
  protected async execute(input: string): Promise<AgentResult> {
    try {
      ...
      const { currentTools, requirements, constraints } = await this.parseInput(input)
    ...
    const commonTools: Tool[] = [
      {
        ...
        capabilities: ['code generation', 'boilerplate creation', 'pattern implementation'],
        limitations: ['requires clear specifications', 'may need manual refinement'],
        dependencies: [],
      ...
      },
      {
        ...
        category: 'testing',
        capabilities: ['endpoint testing', 'response validation', 'performance testing'],
...
Format as JSON with arrays for each category.`
    ...
    const currentTools = parsed.currentTools?.map((toolName: string) => {
      return this.toolLibrary.get(toolName) || {
        ...
        limitations: [],
        dependencies: [],
        cost: { type: 'unknown' as any, estimatedMonthly: 0, scalingFactors: [] },
        performance: {
          speed: 'moderate' as any,
          reliability: 70,
          scalability: 'medium' as any,
          resourceUsage: { cpu: 'medium' as any, memory: 'medium' as any, network: 'medium' as any }
        }
      }
    }) || []
...
Format as JSON array of ToolAddition objects.`
    ...
    return additions.map((addition) => {
      const tool = this.toolLibrary.get(addition.toolId) || {
        ...
        cost: addition.tool.cost || { type: 'unknown', estimatedMonthly: 0, scalingFactors: [] },
