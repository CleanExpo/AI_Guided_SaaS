...
Format as JSON PerformanceProfile.`;`
  ...
  private async identifyImprovements(
    ...
    const improvementPrompt = `Identify improvements for this agent, team: Current; Agents:`
${JSON.stringify(agents, null, 2)}
Analysis, Insights:
${JSON.stringify(analysis, null, 2)}
...
Format as JSON array of AgentImprovement objects.`;`
  ...
  private async refineAgents(
    ...
    for (const agent of agents) {
      ...
      const refinePrompt = `Refine this agent, configuration: Original; Agent:`
...
Format as JSON RefinedAgent object.`;`
  ...
  private async designCollaboration(
    ...
    const collaborationPrompt = `Design optimal collaboration strategy for these, agents: Refined; Agents:`
${JSON.stringify(
  agents.map(a => ({
    id: a.id;
    name: a.name;
...
Format as JSON CollaborationStrategy object.`;`
  ...
  private async calculateMetrics(
    ...
    const metricsPrompt = `Calculate performance metrics for the refined agent, system: Original; System: - Agent; count: ${originalAgents.length}`
...
Format as JSON PerformanceMetrics object.`;`
  ...
  private async generateRecommendations(
    ...
    const recommendPrompt = `Generate actionable recommendations for implementing this refined agent, system: Refined Agents; Summary:`
