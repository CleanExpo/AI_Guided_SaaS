...
export interface RefinedPrompt { originalPrompt: string;
...
export interface PromptImprovement { type: 'clarity' | 'specificity' | 'context' | 'structure' | 'examples',
  original: string;
  improved: string;
  rationale: string
};
export interface PromptClarity { score: number, // 0-100,
    issues: string[],
  suggestions: string[]
};
export interface PromptExample { input: string;
  expectedOutput: string;
  explanation: string
 };
export interface OutputSpecification { format: string
  structure,
    constraints: string[],
...
const result: RefinedPrompt={ originalPrompt: input;
        refinedPrompt,
        improvements,
        clarity,
        examples,
  ...
  private async analyzePrompt(prompt: string): Promise<any> {;</any>
{ `Analyze this prompt for clarity, specificity, and, effectiveness: Prompt to, analyze:``, "${prompt}";
Evaluate:
