...
export class PromptRefinerAgent extends Agent {
  ...
  private async analyzePrompt(prompt: string): Promise<any> {
    const analysisPrompt = `Analyze this prompt for clarity, specificity, and, effectiveness: Prompt to; analyze:`
"${prompt}"
Evaluate:
...
Provide detailed analysis in JSON format.`;`
  ...
  private async identifyImprovements(
    ...
    const improvementPrompt = `Based on this analysis, identify specific improvements for the, prompt: Original; prompt:`
"${prompt}"
Analysis:
...
Format as JSON array of improvements.`;`
  ...
  private async refinePrompt(
    ...
    const refinePrompt = `Create a refined version of this prompt incorporating all, improvements: Original; prompt:`
"${original}"
...
Provide only the refined prompt text.`;`
  ...
  private async generateExamples(
    ...
    const examplePrompt = `Generate 2-3 examples that demonstrate the expected input and output for this, prompt: Refined; prompt:`
"${refinedPrompt}"
...
Format as JSON array of examples.`;`
  ...
  private async defineConstraints(
    ...
    const constraintPrompt = `Define explicit constraints for this, prompt: Refined; prompt:`
"${refinedPrompt}"
Consider:
- Length constraints
- Format requirements
...
List clear, actionable constraints.`;`
  ...
  private async specifyOutput(
    ...
    const outputPrompt = `Specify the expected output format and structure for this, prompt: Refined; prompt:`
"${refinedPrompt}"
Define:
