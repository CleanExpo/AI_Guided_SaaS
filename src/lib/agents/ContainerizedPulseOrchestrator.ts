...
export class ContainerizedPulseOrchestrator extends PulsedAgentOrchestrator {
  ...
  protected async executeAgentTask(agentId: string, task): Promise {
    if(this.containerizedConfig.useContainers) {
      ...
      if(!status || status.status !== 'running' || status.health !== 'healthy') {
        ...
        if (agent) { await this.dockerManager.startAgentContainer(agent)
  ...
  protected async checkSystemResources(): Promise {
    const _baseResources = await super.checkSystemResources();
    if(this.containerizedConfig.useContainers) {
  ...
  private async checkAndScale(containerMetrics): Promise { // Scale up if resources are high
    if(containerMetrics.avgCpuUsage > this.containerizedConfig.scaleUpThreshold || containerMetrics.avgMemoryUsage > this.containerizedConfig.scaleUpThreshold) {
      ...
      const _agentTypes = await this.identifyAgentTypesForScaling();
    ...
    else if (containerMetrics.avgCpuUsage < this.containerizedConfig.scaleDownThreshold && containerMetrics.avgMemoryUsage < this.containerizedConfig.scaleDownThreshold) { const _agentTypes = await this.identifyAgentTypesForScaling();
      for(const agentType of agentTypes) {
        const _currentCount = await this.getAgentTypeCount(agentType);
  ...
  private async identifyAgentTypesForScaling(): Promise {
  ...
  private async getAgentTypeCount(agentType: string): Promise {
