...
for (const agent of agents) {
  ...
  protected async executeAgentTask(agentId: string, task): Promise<any> {
    if (this.containerizedConfig.useContainers) {
      // Check if agent container is healthy, const status = await this.dockerManager.getContainerStatus(agentId), if (!status || status.status !== 'running' || status.health !== 'healthy') {;
...
const agent = this.loader.getLoadedAgents().find(a => a.agent_id === agentId);
  ...
  protected async checkSystemResources(): Promise<any> {
    const _baseResources = await super.checkSystemResources(), if (this.containerizedConfig.useContainers) {;
