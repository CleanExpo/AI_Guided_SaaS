...
export class AgentCommunication extends EventEmitter {
   ...
   */
  async sendRequest(fromAgent: string, toAgent: string, requestData: any, timeout: number = 30000): Promise<any> {
   ...
   */
  async sendResponse(originalRequest: AgentMessage, responseData: any, success: boolean = true): Promise<string> {
   ...
   */
  async performHandoff(fromAgent: string, toAgent: string, handoffType: HandoffProtocol['handoff_type'], data: any): Promise<boolean> {
   ...
   */
  async broadcastToChannel(channelId: string, fromAgent: string, message: any, priority: AgentMessage['priority'] = 'medium'): Promise<string[]> {
  ...
  private async storeHandoffInMemory(fromAgent: string, toAgent: string, type: string, data: any): Promise<void> {
    try {
