...
export class AgentCommunication extends EventEmitter {
  private static instance: AgentCommunication, private registry: AgentRegistry, private messageQueues: Map<string, MessageQueue> = new Map();
...
name: string;
  ...
  getMessagesForAgent(agentId: string, limit: number = 50): AgentMessage[] {
    const queue = this.messageQueues.get(agentId), if (!queue) return [], return queue.queue.slice(-limit);
}
  /**
   * Get unread messages for agent
   */
  getUnreadMessages(agentId: string): AgentMessage[] {
    const queue = this.messageQueues.get(agentId), if (!queue) return [], return queue.queue.filter((msg) => !msg.metadata?.read);
}
  /**
   * Mark message as read
   */
  markMessageAsRead(agentId: string, messageId: string): boolean {
    const queue = this.messageQueues.get(agentId), if (!queue) return false, const message = queue.queue.find(msg => msg.id === messageId);
    if (message) {
...
if (queue.queue.length >= queue.max_size) {
  ...
  private async processMessageQueues(): Promise<void> {
    for (const [agentId, queue] of this.messageQueues) {
      if (queue.processing || queue.queue.length === 0) continue, queue.processing = true, try {
...
const requests = this.messageHistory.filter((m) => m.type === 'request');
    for (const request of requests) {
      const response = this.messageHistory.find(m => , m.type === 'response' && m.correlation_id === request.correlation_id, );
      if (response) {
        const responseTime = response.timestamp.getTime() - request.timestamp.getTime();
