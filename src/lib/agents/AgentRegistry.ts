...
export class AgentRegistry {
  ...
  findAgents(query: RegistryQuery): AgentRegistration[] {
    ...
    for (const registration of Array.from(this.registrations.values())) {
      ...
      if (query.priority_range) {
        const [min, max] = query.priority_range;
  ...
  getBestAgentForTask(taskType: string; requirements: string[] = []): AgentRegistration | null {
    ...
    const scoredCandidates = candidates.map(candidate => {
      ...
      const matchingCapabilities = candidate.agent.capabilities.filter(cap =>;
      ...
      const hoursSinceActive = (Date.now() - candidate.last_heartbeat.getTime()) / (1000 * 60 * 60);
  ...
  updateAgentMetrics(agentId: string, taskResult) {
    ...
    if (taskResult.execution_time) {
      const totalTime = metrics.average_execution_time * (metrics.total_tasks - 1) + taskResult.execution_time;
  ...
  private async verifyAgentCapabilities(agent: AgentConfig): Promise<boolean> {
  ...
  private updateHealthStatus(registration: AgentRegistration) {
    const metrics = registration.metrics;
    const now = Date.now();
    const lastActive = registration.last_heartbeat.getTime();
  ...
  private calculateOverallMetrics(): Record<string, any> {
    ...
    const totalTasks = allMetrics.reduce((sum, m) => sum + m.total_tasks, 0);
    const totalCompleted = allMetrics.reduce((sum, m) => sum + m.completed_tasks, 0);
    const totalExecutionTime = allMetrics.reduce((sum, m) => sum + m.average_execution_time, 0);
    return {
  ...
  private async storeAgentInMemory(registration: AgentRegistration) {
    try {
  ...
  private async logEvent(event: RegistryEvent) {
  ...
  private performHealthChecks() {
    for (const registration of Array.from(this.registrations.values())) {
      const oldStatus = registration.health_status;
      this.updateHealthStatus(registration)
