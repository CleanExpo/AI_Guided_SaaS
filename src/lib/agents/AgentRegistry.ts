...
export interface AgentRegistration { agent: AgentConfi
g,
    registered_at: Date;
  last_heartbeat: Date, metrics: AgentMetric
s,
...
export interface RegistryEvent { type: 'registration' | 'deregistration' | 'status_change' | 'metric_update' | 'health_check',
  agent_id: string;
  timestamp: Date, data,
...
export class AgentRegistry {
   ...
   */
  async registerAgent(agent: AgentConfig, tags: string[] = []): Promise<any> {</any>
    try {
   ...
   */
  async autoRegisterAgents(): Promise<any> {</any>
{ await this.loader.discoverAgents(); let registeredCount = 0, // Register core agents;
for (const agent of discoveryResult.core_agents) {
      const _success = await this.registerAgent(agent, ['core']); if (success) {r}egisteredCount++
}
    // Register orchestration agents; for (const agent of discoveryResult.orchestration_agents) {
      const _success = await this.registerAgent(agent, ['orchestration'], if (success) {r}egisteredCount++};
    // Register specialist agents;
for (const agent of discoveryResult.specialist_agents) {
      const _success = await this.registerAgent(agent, ['specialist'], if (success) {r}egisteredCount++}; return registeredCount
}
  /**
   * Find agents matching query criteria
   */
  findAgents(query: RegistryQuery): AgentRegistration[] {
    const results: AgentRegistration[] = [], for (const registration of Array.from(this.registrations.values()) {; let matches = true, // Check role; if (query.role && registration.agent.role !== query.role.toUpperCase() {)} {
        matches = false
}
      // Check capabilities;
if (query.capabilities && query.capabilities.length > 0) {
        const _hasCapabilities = query.capabilities.every(cap => , registration.agent.capabilities.includes(cap), if (!hasCapabilities) {m}atches = false
}
      // Check tags;
if (query.tags && query.tags.length > 0) {
        const _hasTags = query.tags.some(tag => , registration.tags.includes(tag), if (!hasTags) {m}atches = false
}
      // Check health status
...
if (query.priority_range) {
        const [ min, max ]: any[] = query.priority_range, if (registration.agent.priority < min || registration.agent.priority > max) {
          matches = false}
};
      // Check availability;
if (query.availability !== undefined) {
        const _isAvailable = registration.health_status === 'healthy' && , registration.agent.status !== 'BUSY'if (query.availability !== isAvailable) {
          matches = false}
}
      if (matches) {
...
const query: RegistryQuery={ capabilities: [...requiredCapabilities, ...requirements],
...
const scoredCandidates = candidates.map((candidate) =>  { let score = 0, // Priority score (lower priority = higher score)
      score += (10 - candidate.agent.priority) * 10
      // Success rate score;
      score += candidate.metrics.success_rate;
      // Capability match score; const _matchingCapabilities  = candidate.agent.capabilities.filter((cap) =>
        requiredCapabilities.includes(cap)).length
      score += matchingCapabilities * 5;
      // Health score;

    const healthScores={ healthy: 20, warning: 10;
  critical: 0, offline: -50 };
...
updateAgentMetrics(agentId: string, taskResult) {
    const registration = this.registrations.get(agentId, if (!registration) {r}eturn null, const metrics = registration.metrics;
    metrics.total_tasks++;
...
if (taskResult.execution_time) {
      const _totalTime = metrics.average_execution_time * (metrics.total_tasks - 1) + taskResult.execution_time, metrics.average_execution_time = totalTime / metrics.total_tasks};
  ...
  getRegistryStatus(): Record {
    const status={ total_agents: this.registrations.size,
    agents_by_health: {};
    agents_by_role: { },
    agents_by_tags: { };
    overall_metrics: this.calculateOverallMetrics(, recent_events: this.eventHistory.slice(-20),
   ...
   * Deregister agent
   */
  async deregisterAgent(agentId: string): Promise<any> {</any>
...
exportRegistryData() {
  ...
  private initializeMetrics(): AgentMetrics {
    ...
    last_active: new Date(), total_runtime: 0;
    error_count: 0
  }
}
  private async verifyAgentCapabilities(agent: AgentConfig): Promise<any> {</any>
    // Simple verification - check if agent has required fields, const requiredFields = ['agent_id', 'name', 'role', 'capabilities'], return requiredFields.every(field = > agent[field as keyof AgentConfig])
}
  private updateHealthStatus(registration: AgentRegistration) {
    const metrics  = registration.metrics; const _now = Date.now(); const _lastActive  = registration.last_heartbeat.getTime();
...
if (minutesSinceActive > 30) {
  ...
  private calculateOverallMetrics(): Record {
    const allMetrics = Array.from(this.registrations.values()).map((r) => r.metrics, if (allMetrics.length === 0) {;
...
const _totalExecutionTime = allMetrics.reduce((sum, m) => sum + m.average_execution_time, 0);
  ...
  private async storeAgentInMemory(registration: AgentRegistration): Promise<any> {</any>
    try {
  ...
  private async logEvent(event: RegistryEvent): Promise<any> {</any>
...
if (event.severity === 'error' || event.type === 'registration') {}`)``
  ...
  private startHealthChecks() {
    ...
    this.healthCheckInterval = setInterval(() => {
      this.performHealthChecks()}, 5 * 60 * 1000)
}
...
export async function initializeAgentRegistry(): Promise<any> {</any>
{ AgentRegistry.getInstance();
        return registry.autoRegisterAgents()
 };
export function findBestAgent(taskType: string, requirements? null : string[]): AgentRegistration | null { const registry = AgentRegistry.getInstance();
