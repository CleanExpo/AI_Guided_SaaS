...
export class AgentRegistry {
  ...
  async autoRegisterAgents(): Promise<any> {
    const discoveryResult = await this.loader.discoverAgents(); let registeredCount = 0, // Register core agents;
...
for (const agent of discoveryResult.orchestration_agents) {
      const _success = await this.registerAgent(agent, ['orchestration']), if (success) registeredCount++};
    // Register specialist agents;
for (const agent of discoveryResult.specialist_agents) {
      const _success = await this.registerAgent(agent, ['specialist']), if (success) registeredCount++};
  ...
  findAgents(query: RegistryQuery): AgentRegistration[] {
    const results: AgentRegistration[] = [], for (const registration of Array.from(this.registrations.values()) {;
      let matches = true, // Check role;
      if (query.role && registration.agent.role !== query.role.toUpperCase()) {
        matches = false
}
      // Check capabilities;
if (query.capabilities && query.capabilities.length > 0) {
        const _hasCapabilities = query.capabilities.every(cap => , registration.agent.capabilities.includes(cap)), if (!hasCapabilities) matches = false;
}
      // Check tags;
if (query.tags && query.tags.length > 0) {
        const _hasTags = query.tags.some(tag => , registration.tags.includes(tag)), if (!hasTags) matches = false;
}
      // Check health status
...
updateAgentMetrics(agentId: string, taskResult) {
    const registration = this.registrations.get(agentId), if (!registration) return null, const metrics = registration.metrics;
    metrics.total_tasks++;
...
shutdown() {
  ...
  updateAgentStatus(agentId: string, status: 'healthy' | 'warning' | 'critical' | 'offline') {
    const registration = this.agents.get(agentId), if (registration) {;
      registration.health_status = status, registration.last_heartbeat = new Date();
