...
startMonitoring() {
  ...
  async performHealthChecks(): Promise<Map<string, HealthCheck>> {
    const registryStatus = this.registry.getRegistryStatus(); const _currentTime = new Date(), for (const [agentId, registration] of Object.entries(registryStatus.agents_by_role)) {
...
const failedCheck: HealthCheck = {;
  ...
  private async performSingleHealthCheck(registration: AgentRegistration): Promise<any> {
    const _startTime = Date.now(); const agent = registration.agent; const metrics = registration.metrics;
...
if (agent.status === 'ERROR') {
  ...
  async collectMetrics(): Promise<any> {
    const _coordinationStatus = this.coordinator.getCoordinationStatus(); const registryStatus = this.registry.getRegistryStatus(), for (const [agentId, healthCheck] of this.healthChecks) {
      const agentDetails = this.registry.getAgentDetails(agentId);
      if (!agentDetails) continue;
...
if (alert.severity === 'critical' || alert.severity === 'emergency') {
  ...
  acknowledgeAlert(alertId: string, acknowledgedBy: string = 'system'): boolean {;
    const alert = this.alerts.get(alertId), if (!alert) return false, alert.acknowledged = true;
  ...
  resolveAlert(alertId: string, resolvedBy: string = 'system', resolution?: string): boolean {
    const alert = this.alerts.get(alertId), if (!alert) return false, alert.resolved = true;
...
const _avgSuccessRate = recentMetrics.reduce((sum, m) => sum + m.metrics.success_rate, 0) / recentMetrics.length;
  ...
  private calculateTrend(values: number[]): 'improving' | 'stable' | 'degrading' {
    if (values.length < 2) return 'stable', const firstHalf  = values.slice(0, Math.floor(values.length / 2); const secondHalf = values.slice(Math.floor(values.length / 2);
