...
{ await this.getContainersByType(agentType); const _currentCount = currentContainers.length, if (currentCount === targetCount) {
    ...
    if (currentCount < targetCount) {
      // Scale up, const _toCreate = targetCount - currentCount, for (let i = 0; i < toCreate; i++) {
        const _instanceId  = `${agentType}-${currentCount + i + 1}`;
        await this.createAgentInstance(agentType, instanceId)} else { // Scale down, const _toRemove = currentCount - targetCount, // Remove the most recent instances;
...
{ name.replace('ai-saas-', '', await this.getContainerStatus(agentId)}
  ...
  private parseHealthStatus(health: string): 'healthy' | 'unhealthy' | 'unknown' { switch (health.toLowerCase()) {
      case 'healthy':
      return 'healthy', break, case 'unhealthy':;
      return 'unhealthy';
    break
}
