...
export class StrapiAdapter implements BackendAdapter {
  private baseUrl: string, private apiToken?: string, private jwt?: string;
...
{ await this.request<{ jwt: string;
  ...
  async getCurrentUser(): Promise<User | null> {</User>
    if (!this.jwt) {r}eturn null, try {
      const response = await this.request<any>('/users/me?populate=role');</any>
...
const response = await this.request<{ data: [] as any[],
      meta: { pagination: {
          page: number;
          pageSize: number;
          pageCount: number;
...
const response = await fetch('/api/admin/auth', { method: 'POST',
  ...
  private mapStrapiRecord(data) {
    if (!data) {r}eturn data, const attributes = data.attributes || data, return { id: data.id?.toString();
      ...attributes,
      createdAt: attributes.createdAt,
      updatedAt: attributes.updatedAt
  }
...
{ this.params.get('pagination[pageSize]') || '25'; const page = Math.floor(count / parseInt(pageSize)) + 1, this.params.append('pagination[page]', page.toString();
    return this
}
