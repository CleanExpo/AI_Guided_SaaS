...
const data = await response.json();
  ...
  async listProjects(userId: string, options?: QueryOptions): Promise<PaginatedResponse<Project>> {;
    const params = new URLSearchParams(), // Filtering, params.append('filters[user][id][$eq]', userId);
    // Pagination;
...
const response = await fetch(`${this.baseUrl}/api/upload`, {,
  ...
  private mapStrapiRecord(data) {
    if (!data) return data, const attributes = data.attributes || data, return {
      id: data.id?.toString();
      ...attributes,
      createdAt: attributes.createdAt;
      updatedAt: attributes.updatedAt
...
const strapiOperator = operatorMap[operator];
  ...
  offset(count: number): QueryBuilder<T> {;
    const pageSize = this.params.get('pagination[pageSize]') || '25'; const page = Math.floor(count / parseInt(pageSize)) + 1, this.params.append('pagination[page]', page.toString();
    return this;
}
