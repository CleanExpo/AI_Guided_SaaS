import {  BackendAdapter, User, Project, QueryOptions, PaginatedResponse, QueryBuilder, DatabaseEvent, BackendConfig, BackendError  } from '../types';
  ...
  // Authentication
  async signUp(email: string, password: string, metadata?): Promise {
    const response = await this.request<{
    jwt: string
      user}>('/auth/local/register', { method: 'POST'
}
  body: JSON.stringify({
  username: email,
        email,
  ...
  identifier: email,
    ...
    this.jwt = response.jwt
    return { user: this.mapStrapiUser(response.user),;
}
  token: response.jwt
}
}
  ...
  // Projects
  async createProject(data: Omit<Project, 'id' | 'createdAt' | 'updatedAt'>): Promise {
    const response = await this.request<{ data}>('/projects', { method: 'POST',
  ...
  async listProjects(userId: string, options?: QueryOptions): Promise<PaginatedResponse<Project>> {
    ...
    // Sorting
    if (options?.orderBy) {
      const _sort = `${options.orderBy}:${options.order || 'asc'}`
  ...
  pageCount: number,
   total: number
    ...
    return { data: (response as any).data.map(this.mapStrapiProject),;
  ...
  // Generic CRUD
  async create<T>(collection: string, data): Promise {
  ...
  pageCount: number,
   total: number
    ...
    return { data: (response as any).data.map(this.mapStrapiRecord) as T[],;
  ...
  // Helper methods
  private mapStrapiUser(user): User { return {;
      id: user.id.toString(),
  ...
  private mapStrapiProject(data): Project { const attributes = data.attributes || data;
    return {;
      id: data.id.toString(),
  ...
  private mapStrapiRecord(data) { if (!data) return data;
...
class StrapiQueryBuilder<T> implements QueryBuilder {
  ...
  where(field: string,
   operator: string, value): QueryBuilder {
    ...
    const _strapiOperator = operatorMap[operator];
    if (!strapiOperator) {
