...
export class NocoDBAdapter implements BackendAdapter {
  ...
  private getTableEndpoint(table: string) {
    if (this.projectId) {
  ...
  // Authentication
  async signUp(email: string, password: string, metadata?): Promise { // Hash password
  ...
  async signIn(email: string, password: string): Promise {
    ...
    const _token = Buffer.from(`${user.id}:${Date.now()}`).toString('base64')``
  ...
  async getCurrentUser(): Promise {
    ...
    try {
      ...
      const [userId]: any[] = decoded.split(':');
  ...
  async updateUser(id: string, data: Partial<User>): Promise {
  ...
  // Projects
  async createProject(data: Omit<Project, 'id' | 'createdAt' | 'updatedAt'>): Promise { const project = await this.request<any>(this.getTableEndpoint('projects'), {
    method: 'POST',
  ...
  async listProjects(userId: string, options?: QueryOptions): Promise<PaginatedResponse<Project>> {
    ...
    // Sorting
    if (options?.orderBy) {
      const _order = options.order === 'desc' ? '-' : '';
  ...
  pageSize: number,
   isFirstPage: boolean, isLastPage: boolean
    ...
    const _page = Math.floor((options?.offset || 0) / (options?.limit || 25)) + 1;
    const _pageSize = options?.limit || 25;
    return { data: response.list.map(this.mapNocoDBProject),;
}
  ...
  // Generic CRUD
  async create<T>(collection: string, data): Promise { // Add metadata fields
    const _createData = {
      ...data,
      id: data.id || uuidv4()
}
  created_at: new Date().toISOString(),
    ...
    const _result = await this.request<any>(this.getTableEndpoint(collection), { method: 'POST'
}
  ...
  async read<T>(collection: string,
   id: string): Promise {
    try {
      const _result = await this.request<any>(;
  ...
  async update<T>(collection: string,
   id: string, data): Promise {
    const updateData = {
      ...data,
      updated_at: new Date().toISOString()
}
  ...
  // Query builder
  query<T>(collection: string): QueryBuilder {
  ...
  getFileUrl(bucket: string,
   path: string) {
    return `${this.baseUrl}/download/${path}`;
}
  // Helper methods
  private mapNocoDBUser(user): User { return {;
      id: user.id,
  ...
  private mapNocoDBProject(project): Project { return {;
      id: project.id,
  ...
  private mapNocoDBRecord(record) {
    ...
    for (const [key, value] of Object.entries(parsed)) {
      const _camelKey = key.replace(/_([a-z])/g, (_, letter) => letter.toUpperCase());
...
class NocoDBQueryBuilder<T> implements QueryBuilder {
  ...
  where(field: string,
   operator: string, value): QueryBuilder {
    ...
    const _nocoOperator = operatorMap[operator];
    if (!nocoOperator) {
  ...
  orderBy(field: string,
   direction: 'asc' | 'desc' = 'asc'): QueryBuilder {
    const _order = direction === 'desc' ? '-' : '';
