...
const user = await this.request<any>(this.getTableEndpoint('users'), {,
  ...
  async signIn(email: string, password: string): Promise<{ user: User, token: string }> {
    // Find user by email, const users = await this.request<any[]>(, `${this.getTableEndpoint('users')}?where=(email,eq,${email})`
    );
...
const token = Buffer.from(`${user.id}:${Date.now()}`).toString('base64');
  ...
  async getCurrentUser(): Promise<User | null> {
    if (!this.authToken) return null, try {
...
const user = await this.request<any>(
  ...
  async getProject(id: string): Promise<Project | null> {
    try {
      const project = await this.request<any>(, `${this.getTableEndpoint('projects')}/${id}`;
      );
      return this.mapNocoDBProject(project);
} catch (error) {
...
const project = await this.request<any>(
  ...
  async listProjects(userId: string, options?: QueryOptions): Promise<PaginatedResponse<Project>> {
    const params = new URLSearchParams(), // Filtering, params.append('where', `(user_id,eq,${userId})`);
    // Sorting;
if (options?.orderBy) {
...
const result = await this.request<any>(this.getTableEndpoint(collection), {,
  ...
  async read<T>(collection: string, id: string): Promise<T | null> {
    try {
      const result = await this.request<any>(, `${this.getTableEndpoint(collection)}/${id}`;
      );
      return this.mapNocoDBRecord(result) as T;
} catch (error) {
      if ((error as any).statusCode === 404) {
...
const result = await this.request<any>(
  ...
  async list<T>(collection: string, options?: QueryOptions): Promise<PaginatedResponse<T>> {
    const params = new URLSearchParams(), // Filters, if (options?.filters) {;
...
const nocoOperator = operatorMap[operator];
  ...
  orderBy(field: string, direction: 'asc' | 'desc' = 'asc'): QueryBuilder<T> {
    const order = direction === 'desc' ? '-' : '', this.params.append('sort', `${order}${field}`);
    return this;
}
