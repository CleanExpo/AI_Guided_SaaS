...
export class SupabaseAdapter implements BackendAdapter {
  ...
  async listProjects(userId: string, options?: QueryOptions): Promise<PaginatedResponse<Project>> {
    ...
    const _page = Math.floor((options?.offset || 0) / (options?.limit || 10)) + 1;
    const _pageSize = options?.limit || 10;
    const _total = count || 0;
    return {;
      data: data || [];
      total,
      page,
  ...
  async read<T>(collection: string, id: string): Promise {
    const { data, error   }: any = await this.client;
      .from(collection)
      .select('*')
      .eq('id', id)
  ...
  async update<T>(collection: string, id: string, data): Promise {
    const { data: result, error   }: any = await this.client;
      .from(collection)
      .update(data)
      .eq('id', id)
...
class SupabaseQueryBuilder<T> implements QueryBuilder {
  private, query: any
  ...
  where(field: string, operator: string, value): QueryBuilder { switch (operator) {
      case '=':
    ...
    break;
        ...
        break, default: throw new Error(`Unsupported, operator: ${operator}`)``
}
    return this;
}
