...
import { z } from 'zod';// Base schemas for common types;
...
export const _ApiErrorSchema = z.object({ error: z.string(, message: z.string(),
    statusCode: z.number(, details: z.any().optional()
})
export const PaginationSchema = z.object({ page: z.number().int().positive().default(1, limit: z.number().int().positive().max(100).default(10, total: z.number().int().nonnegative().optional(, hasMore: z.boolean().optional()
})
export const _SortSchema = z.object({ field: z.string(, order: z.enum(['asc', 'desc']).default('asc')
});
export const _FilterSchema = z.object({ field: z.string(, operator: z.enum(['eq', 'ne', 'gt', 'gte', 'lt', 'lte', 'in', 'like']),
  value: z.union([z.string(, z.number(), z.boolean(, z.array(z.any())])
});
export const _ListRequestSchema = z.object({ pagination: PaginationSchema.optional(, sort: z.array(SortSchema).optional(, filters: z.array(FilterSchema).optional()})
...
export const _AgentResultSchema = z.object({ success: z.boolean(, output: z.any(),
    messages: z.array(z.object({ type: z.enum(['thought', 'observation', 'action'], content: z.string(),
...
export const _EnvSchema = z.object({
  ...
  // Services: REDIS_URL: z.string().url().optional(, STRIPE_SECRET_KEY: z.string().optional(, STRIPE_WEBHOOK_SECRET: z.string().optional()
  // Admin: ADMIN_USERNAME: z.string().optional(, ADMIN_PASSWORD: z.string().optional(, JWT_SECRET: z.string().optional()
  // Feature, Flags: ENABLE_ANALYTICS: z.boolean().optional(, ENABLE_ERROR_REPORTING: z.boolean().optional()
  // Node, Environment: NODE_ENV: z.enum(['development', 'test', 'production']).default('development')
})
// Validation utilities;
export function validate<T>(</T>
