...
export class ClientRequirementsProcessor {;
  ...
  constructor(aiService: AIService; agentCoordinator: AgentCoordinator) {
    ...
    try {
      const templatePath = path.join(process.cwd(), 'prp_templates', 'ClientSpecExtraction.prp');
      this.extractionTemplate = readFileSync(templatePath, 'utf-8')
    } catch (error) {
  ...
  private async extractRequirements(input: string): Promise<ExtractedRequirement[]> {
    const prompt = `;`
...
``
  ...
  private determineAgents(requirement): string[] {
    ...
    // Use existing agents if provided
    if (Array.isArray(requirement.agents)) {
  ...
  private determineComplexity(
    ...
    const score = requirements.reduce((total, req) => {;
      let points = 0;
      // Priority scoring
      if (req.priority === 'high') points += 3
  ...
  private estimatePhaseDuration(requirements: ExtractedRequirement[]): string {
    const points = requirements.reduce((total, req) => {;
      let p = 1;
  ...
  private estimateDuration(
    ...
    const weeks = baseWeeks[complexity];
  ...
  private extractProjectName(input: string): string {
    ...
    const nameMatch = input.match(/(?:project|app|application|platform|system)\s+(?:called|named)?\s*"?([^"]+)"?/i);
  ...
  private async validateRequirements(requirements: ExtractedRequirement[]): Promise<void> {
    // Check for conflicts
  ...
  private findMissingDependencies(requirements: ExtractedRequirement[]): string[] {
    ...
    const hasBackend = requirements.some(r => r.agents.includes('agent_backend'));
...
## Categories: -; Functional: Core features and capabilities
