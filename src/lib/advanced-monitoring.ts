...
class AdvancedMonitoringSystem {
  ...
  getSecurityAnalytics(): {
    ...
    const _topAttackTypes = Object.entries(attackTypes);
  ...
  getPerformanceAnalytics(): {
    ...
    const _slowestEndpoints = Object.entries(endpointTimes);
      .map(([endpoint, times]) => ({
        endpoint, avgTime: times.reduce((sum, time) => sum + time, 0) / times.length}))
  ...
  private trimHealthHistory() { // Keep only last 1000 health records
