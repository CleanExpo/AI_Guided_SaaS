...
import { logSecurity, logWarn } from './production-logger';
...
class AdvancedMonitoringSystem {
   ...
   */
  trackPerformance(metric: Omit<PerformanceMetric, 'timestamp' | 'memoryUsage' | 'cpuUsage'>): void {
  ...
  getSecurityAnalytics(): {
    ...
    const attackTypes = recentEvents.reduce((acc: Record<string, number>, event) => {
  ...
  getPerformanceAnalytics(): {
    ...
    const endpointTimes = recentMetrics.reduce((acc: Record<string, number[]>, metric) => {
    ...
    const memoryTrend = this.systemHealth
  ...
  private triggerPerformanceAlert(message: string, details: Record<string, unknown>): void {
    const alert = {
      type: 'performance',
      message,
  ...
  private generateRecommendations(
    ...
    if (health.memoryUsage.percentage > 0.8) {
  ...
  private trimHealthHistory(): void {
    ...
    if (this.systemHealth.length > 1000) {
