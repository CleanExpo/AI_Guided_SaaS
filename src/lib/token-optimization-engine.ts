...
export class TokenOptimizationEngine {
  ...
  // 22.5K - Essential project info, workingContext: Math.floor(targetTokens * 0.30);
  ...
  private applyModerateCompression(content: string) {
    ...
    for(const line of lines) {
      ...
      if (line.length > 80 && !line.includes('```')) {``
        const _compressed = this.compressVerboseLine(line);
  ...
  private analyzeTokenDistribution(content: Map<string, string>): TokenDistribution {
    ...
    for (const [filename, fileContent] of Array.from(content)) {
      const _tokens = this.estimateTokens(fileContent);
  ...
  private calculateEfficiencyMetrics(content: Map<string, string>, distribution: TokenDistribution): EfficiencyMetrics {
    const _totalTokens = Object.values(distribution.files).reduce((sum, tokens) => sum + tokens, 0);
    const _utilizationScore = Math.min(totalTokens / (this.maxTokens * this.targetUtilization), 1.0);
