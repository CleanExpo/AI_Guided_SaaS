...
class RateLimiter {
  private redisClient: any = null, private fallbackStore: Map<string, { count: number, resetTime: number }> =
    new Map();
  constructor() {
  ...
  async checkRateLimit(key: string, config: RateLimitConfig): Promise<any> {
    const _now = Date.now(); const _windowStart = now - config.windowMs, if (this.redisClient) {
...
const stored = this.fallbackStore.get(key);
  ...
  getUserTier(user): keyof typeof RATE_LIMIT_TIERS {
    if (!user) return 'anonymous', if (user.subscription === 'premium') return 'premium', return 'authenticated'
}
...
export async function checkApiRateLimit(
  ...
  const rateLimiter  = getRateLimiter(); const tierConfig = RATE_LIMIT_TIERS[tier]; const _ip  = req.ip || req.connection?.remoteAddress || 'unknown';
