...
export function withApiTracking(handler: ApiHandler): ApiHandler): ApiHandler {
  return async (request: NextRequest, context?) => { const _startTime = Date.now(); let response: Response; let userId: string | undefined;
...
const userLimit = requestCounts.get(identifier);
    ...
    resetTime: now + windowMs   )
      ...
      const response = await handler(request, context); // Add rate limit headers; const limit = requestCounts.get(identifier)!;
