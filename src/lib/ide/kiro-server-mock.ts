...
const _sampleFiles = new Map([;
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24">, <h1 className="text-4xl font-bold">Welcome to Sample Project</h1>
...
export const metadata: Metadata={ title: 'Sample Project',
...
export class KiroServerMock {
  // Project operations
  static async createProject(project: Omit<KiroProject 'id' | 'createdAt' | 'updatedAt'>): Promise<any> {</any>
{ Math.random().toString(36).substring(7); const newProject: KiroProject={
      ...project;
      id,
      createdAt: new Date().toISOString(, updatedAt: new Date().toISOString()};
  ...
  static async getProject(projectId: string): Promise<any> {</any>
{ mockProjects.get(projectId, if (!project) {
      throw new Error('Project not found')};
    return project
}
  static async listProjects(): Promise<any> {</any>
    return Array.from(mockProjects.values())}
  static async saveProject(projectId: string): Promise<any> {</any>
{ mockProjects.get(projectId, if (!project) {
      throw new Error('Project not found')}
    project.updatedAt = new Date().toISOString()
}
  // File operations
  static async readFile(projectId: string, path: string): Promise<any> {</any>
{ mockFiles.get(projectId, if (!projectFiles) {
      throw new Error('Project not found')};
...
{ mockFiles.get(projectId, if (!projectFiles) {
  ...
  static async deleteFile(projectId: string, path: string): Promise<any> {</any>
{ mockFiles.get(projectId, if (!projectFiles) {
      throw new Error('Project not found')}
...
{ mockFiles.get(projectId, if (!projectFiles) {
  ...
  // Terminal operations
  static async createTerminal(config? null : Partial<KiroTerminal>): Promise<any> {</any>
{ Math.random().toString(36).substring(7); const terminal: KiroTerminal={
      id;
...
shell?: config.shell || '/bin/bash',
  ...
  static async closeTerminal(terminalId: string): Promise<any> {</any>
    mockTerminals.delete(terminalId)}
  // AI assistance
  static async getAISuggestions(projectId: string, file: string): Promise<any> {</any>
    // Mock AI suggestions
    return { suggestions: [
        {
  id: 'suggest-1',
  ...
  static async getCompletions(file: string, position: { line: number, character: number }): Promise<any> {</any>
    // Mock code completions
    return [{ label: 'useState',
        kind: 'function',
        detail: 'React Hook',
        documentation: 'Returns a stateful value, and a function to update it.',
        insertText: 'useState<any>(${ 1: initialState})'</any>
  } { label: 'useEffect',
        kind: 'function',
        detail: 'React Hook',
        documentation:
  ...
  static async runDiagnostics(projectId: string): Promise<any> {</any>
    // Mock diagnostics
