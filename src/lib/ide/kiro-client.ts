...
export interface KiroProject { id: string;
  name: string;
  description?,
...
export interface KiroMarker { severity: 'error' | 'warning' | 'info' | 'hint',
  message: string;
  startLine: number;
  startColumn: number;
  endLine: number;
...
export interface KiroTerminal { id: string;
  name: string;
  shell?: string,
  cwd?: string,
...
export interface KiroSuggestion { id: string;
...
export interface KiroDiagnostic { file: string;
  line: number;
  column: number;
...
export interface KiroRefactoring { id: string;
  name: string;
  description: string;
...
export interface KiroCompletion { label: string;
...
interface' | 'module' | 'property';
  ...
  insertText: string;
  range? null : { start: { line: number;
  character: number
}
  end: { line: number, character: number }}

export interface KiroCodeChange { file: string;
  changes: Array<{ range: {
  start: { line: number;
  character: number
}
...
export function getKiroClient(config? null : KiroConfig): KiroConfig): KiroClient {
