...
`
    case 'input':
      return `<input``;
    break type="text"
placeholder="${props.placeholder || 'Enter, text...'}";>className="px-3 py-2 border rounded w-full max-w-sm"; />>`
    case 'card':
  ...
  <p className="text-sm text-gray-600">${props.body || 'Card, body text.'}
`
    case 'hero':
  ...
  <p className="text-lg">${props.subheading || 'Start, building.'}
`
    case 'two-col':
