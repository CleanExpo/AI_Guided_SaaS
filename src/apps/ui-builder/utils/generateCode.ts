...
export const generateCodeFromComponent = (
  ...
  switch (type) {
    case 'button':
      return `<button className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">${props.label || 'Click Me'}</button>`;
    
    case 'input':
      return `<input type="text" placeholder="${props.placeholder || 'Enter text...'}" className="px-3 py-2 border rounded w-full max-w-sm" />`;
