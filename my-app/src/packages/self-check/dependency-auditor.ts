...
export function auditDependencies(): { status: string; outdated: string[]; summary: string } {
  try {
    ...
    try {
      ...
      if (errorOutput) {
        try {
          const outdated: Record<string, DependencyInfo> = JSON.parse(errorOutput);
          const outdatedList = Object.entries(outdated).map(([name, info]) => 
            `⚠️ ${name}: ${info.current} → ${info.latest}`
          );
...
export function getSecurityAudit(): { status: string; vulnerabilities: string[]; summary: string } {
  try {
