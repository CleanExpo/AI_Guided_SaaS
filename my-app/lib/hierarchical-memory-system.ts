...
export class HierarchicalMemorySystem {
  ...
  constructor() {
    this.userMemory = {
      name: 'User Memory',
      maxTokens: 15000, // ~/.claude/CLAUDE.md personal preferences
      currentTokens: 0,
      priority: 'critical',
      retentionPolicy: {
        type: 'hybrid',
    ...
    this.projectMemory = {
      name: 'Project Memory',
      maxTokens: 50000, // ./CLAUDE.md team-shared standards
      currentTokens: 0,
      priority: 'high',
      retentionPolicy: {
        type: 'priority',
    ...
    this.modularMemory = {
      name: 'Modular Memory',
      maxTokens: 135000, // @path/to/import modular files
      currentTokens: 0,
      priority: 'medium',
      retentionPolicy: {
        type: 'frequency',
  ...
  private async compactEntry(entry: MemoryEntry): Promise<MemoryEntry> {
    ...
    return {
      ...
      metadata: {
        ...entry.metadata,
        size: Math.floor(entry.metadata.size * 0.6), // 40% compression
        lastAccessed: new Date()
      }
    }
  }
