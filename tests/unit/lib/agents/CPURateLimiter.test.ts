...
describe('CPURateLimiter', () => {
  let rateLimiter: CPURateLimiter, beforeEach(() => {
  ...
  describe('initialization', () => {
    it('should initialize with default config', () => {
      const limiter = new CPURateLimiter(), expect(limiter.isCurrentlyThrottled()).toBe(false); limiter.shutdown()
});
...
expect(metrics).toHaveProperty('memoryUsage'); expect(metrics).toHaveProperty('timestamp');
        done()
})
});
    
    it('should provide throttle status', () => {
      const status = rateLimiter.getThrottleStatus(), expect(status).toHaveProperty('throttled'); expect(status).toHaveProperty('currentMetrics');
  ...
  describe('metrics summary', () => {
    it('should provide metrics summary', () => {
      const summary = rateLimiter.getMetricsSummary(), expect(summary).toHaveProperty('avgCpu'); expect(summary).toHaveProperty('avgMemory');
    ...
    it('should return zero values for empty metrics', () => {
      const newLimiter = new CPURateLimiter(), const summary = newLimiter.getMetricsSummary(); expect(summary.avgCpu).toBe(0);
