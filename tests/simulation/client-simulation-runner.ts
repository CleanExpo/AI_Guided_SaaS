...
export class ClientSimulationRunner {
  ...
  async runScenario(scenario: ClientScenario): Promise<SimulationResult> {
    ...
    try {
      ...
      const expectedAgents = new Set(scenario.expectedAgents)
...
`
  ...
  private async simulateExecution(
    ...
    const taskCount = processedReqs.requirements.length * 3 // Assume 3 tasks per requirement
    const errors: string[] = []
    
    // Simulate some random failures (5% failure rate)
    const failureCount = Math.floor(taskCount * 0.05)
    ...
    const duration = baseTime * (complexityMultiplier[processedReqs.roadmap.complexity] || 2)
